...
> Generated by scribe for run `REQ-cv-method-phase2` on 2026-03-16. Previous: `REQ-score-unify-001` (2026-03-16).
...
| `R/fect_nevertreated.R` | Estimator | Nevertreated predictive routine for IFE and CFE; co-only estimation with three-layer projection and block coordinate descent for treated parameters. Now accepts `cv.method` with LOO, all_units, treated_units; hardcoded 1% selection rule; W/count.T.cv passed to scoring | `fect_nevertreated()` (internal) | **yes** — cv.method, W.tr, count.T.cv, 1% rule |
...
graph TD
    ...
    cv_map -->|loo + nevertreated| fect_nt["fect_nevertreated()"]
    cv_map -->|all_units / treated| cv_internal["cv.sample + score"]

    fect_boot --> fect_cv2["fect_cv(cv.method)"]

    fect_nt --> score_nt[".score_residuals()"]
...
> Shows how `cv.method` flows from user-facing `fect()` through all CV-related functions. `fect_cv` delegates to `fect_nevertreated` when `cv.method="loo"` + nevertreated, otherwise handles cv.sample masking internally. All paths converge on `.score_residuals()`.
...
- The cv.sample-based CV path in `fect_nevertreated` for `cv.method="all_units"` and `"treated_units"` has parameter/validation infrastructure in place, but the actual cv.sample loop logic (hold out obs, re-estimate, score) is not yet fully implemented within the r-search loop. The existing LOO path continues to handle these cases. The parameter infrastructure is ready for future implementation.
