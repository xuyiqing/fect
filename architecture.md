...
> Generated by scribe for run `REQ-book-cleanup` on 2026-03-17. Previous: `REQ-cv-sample-nevertreated` (2026-03-16).
...
| `R/esplot.R` | Output | Standalone event-study plot from user-supplied data frames; cex parameters now use multiplier pattern matching plot.fect() | `esplot()` | **yes** — cex multiplier fix |
...
| `vignettes/` | Docs | Quarto book: 12 chapters (see table below). R scripts in `rscript/` aligned to code chapters (8 scripts: 02-09). Old superseded chapters deleted. | (docs) | **yes** — major restructure (REQ-book-cleanup) |
...
graph TD
    ...
    style esplot_fn fill:#1e90ff,stroke:#1565c0,color:#fff
```

> Traces from public entry points through R-side orchestration. Diamond nodes represent method dispatch branch points. Blue nodes were modified in this run or the current run (cv.method threading; esplot cex fix).
...
- **esplot() cex multiplier fix (REQ-book-cleanup)**: `esplot()` now treats user-supplied `cex.main`, `cex.lab`, `cex.axis`, and `cex.text` as multipliers (base * user_value), matching `plot.fect()` behavior. Previously, user values were passed through as absolute pt values, making `esplot(..., cex.main = 1.2)` produce 1.2pt text instead of the expected 19.2pt. Default behavior (NULL args) is unchanged.

- **Quarto book restructure (REQ-book-cleanup)**: Major reorganization from 13 chapters to 12 chapters. Key changes: (1) Ch2 restructured into 6 sections (Simulated data, FEct estimator, Visualization & diagnostics, Other estimands, Additional notes); (2) Ch3 renamed "Low-Rank Factor Methods"; (3) Ch5 (Diagnostics) dissolved into Ch2 (brief placebo), Ch3 (full suite), Ch4 (kept existing); (4) Ch6 (Model Selection) dissolved into Ch3 (CV), Ch4 (fect_mspe), Ch7 (availability table); (5) Effect Heterogeneity promoted from Ch2 section to standalone Ch5; (6) chapters renumbered 07->06, 08->07, 09->08, 10->09; (7) esplot.R cex multiplier reverted (was double-multiplying from plot.fect). 5 old superseded chapter files deleted. R scripts in `rscript/` realigned to 8 scripts (02-09).

### Quarto Book Structure

| File | Chapter | Title | R Script |
| --- | --- | --- | --- |
| `index.qmd` | — | Welcome! | — |
| `01-start.Rmd` | 1 | Get Started | — |
| `02-fect.Rmd` | 2 | The Imputation Estimator | `rscript/02-fect.R` |
| `03-ife-mc.Rmd` | 3 | Low-Rank Factor Methods | `rscript/03-ife-mc.R` |
| `04-cfe.Rmd` | 4 | Complex Fixed Effects | `rscript/04-cfe.R` |
| `05-hte.Rmd` | 5 | Effect Heterogeneity | `rscript/05-hte.R` |
| `06-plots.Rmd` | 6 | Plot Options | `rscript/06-plots.R` |
| `07-gsynth.Rmd` | 7 | Gsynth Program | `rscript/07-gsynth.R` |
| `08-panel.Rmd` | 8 | Modern DID Methods | `rscript/08-panel.R` |
| `09-sens.Rmd` | 9 | Sensitivity Analysis | `rscript/09-sens.R` |
| `aa-cheatsheet.Rmd` | A | Cheatsheet | — |
