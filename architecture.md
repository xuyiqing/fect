...
> Generated by scribe for run `REQ-test-gaps-001` on 2026-03-16. Previous: `REQ-cfe-cv-rselect-001` (2026-03-16).
...
| `tests/testthat/` | Tests | Test suite: basic FE/IFE/CFE simulations, staggered designs, normalization, plots, edge cases, DID wrapper, sensitivity, utility function coverage (fect_mspe, esplot, att.cumu, effect) | (test) | **yes** |
...
| `fect.default()` | `default.R` | `fect.formula()`, user | `initialFit()`, estimators, `fect_cv()`, `fect_boot()`, `fect_test()`, `diagtest()`, `fect_permu()` | no | Core orchestrator: validates inputs, reshapes long-to-wide, normalizes, dispatches estimator and inference. Includes `cfe+nevertreated` dispatch branch and `factors.from` passthrough to `fect_boot()` |
...
| `fect_nevertreated()` | `fect_nevertreated.R` | `fect.default()`, `fect_boot()` | `inter_fe_ub()`, `complex_fe_ub()`, `fect_cv()` | no | Nevertreated predictive routine for IFE and CFE; method bifurcation, three-layer projection, block coordinate descent for treated parameters |
| `fect_boot()` | `boot.R` | `fect.default()` | `fect_fe()`, `fect_cfe()`, `fect_mc()`, `fect_nevertreated()` | no | Bootstrap/jackknife inference; parallel via future/doFuture. Includes `cfe+nevertreated` bootstrap dispatch and CFE array subsetting |
...
- **Known bug in `.as_mask()` (R/fect_mspe.R:226)**: The internal helper `.as_mask()` uses `as.logical(mask)` which strips matrix dimensions, returning a flat vector. Downstream code at line 282 (`mask_mat[cbind(rr, cc)]`) expects a matrix, causing `fect_mspe_sim()` to fail for its primary use case (simulation-based MSPE with Y0 ground truth). The input validation paths (Y0 check, hide_mask check) work because they execute before the buggy code path. Discovered during REQ-test-gaps-001.

- **CFE CV r-selection diagnostic (REQ-cfe-cv-rselect-001)**: The CFE CV algorithm for factor count selection is structurally correct. A reported issue where CFE CV appeared to misselect r was traced to test misspecification: the test used `make_cfe_z_data` (DGP with Z*gamma interaction) but did not pass `Z = "Z"` to `fect()`. Without Z specified, factors absorb the unmodeled Z*gamma component (a rank-1 outer product), inflating the selected r by approximately 1. When Z is properly specified, `complex_fe_ub` estimates gamma and subtracts Z*gamma from residuals before CV, and the MSPE shows a clear U-shape with minimum at the true r. This is expected statistical behavior (omitted variable bias in factor selection), not an algorithm bug.
