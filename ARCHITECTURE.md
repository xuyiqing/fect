...
> B8b-B8e) and C1-C6 listed in `NEWS.md` under fect 2.4.6.
...
| --- | --- | --- | --- | --- |
| `R/default.R` (4,104 lines) | API + Validation | `fect()` generic, formula and default methods: input checks, reshaping to T x N matrices, CV / fit / bootstrap routing, diagnostics, output assembly | `fect()` | yes: A2 (stores `vartype`), A6 (`cl` checks, parametric warning), B5 (`wgt.implied` dimnames), B7, B8, B8b-d (loo refit arguments), B9, C1-C6 |
...
| `src/auxiliary.cpp` (413) | C++ | EM helpers, `XXinv()`, `wXXinv()` | internal | yes: C5 (pseudo-inverse when X'X is singular, rcond <= 1e-12) |
...
| `.fect_data_long()` | `R/default.R` | `fect.default()` (output) | — | new (C4) | `data.long` with a factor time column in level order when the time index had non-numeric levels |
...
| `XXinv()`, `wXXinv()` | `src/auxiliary.cpp` | C++ cores | Armadillo | yes (C5) | `inv()` / `inv_sympd()` when rcond > 1e-12, else `pinv()` |
...
| time index | `fect.default()` right after `time <- index[2]` | Factor: numeric levels become numbers, other levels become positions with `time.labels`; character must parse as numbers (C4) |
...
output. The C++ `XXinv()` / `wXXinv()` pseudo-inverse fallback is a second guard for rank-deficient
bootstrap resamples; well-conditioned fits take the old code path byte for byte.
...
- **R/C++ layering**: numerically heavy steps (SVD, EM, demeaning) live in C++; R handles control
  ...
  check; C++ `panel_factor()` clamp and `XXinv()` pseudo-inverse).
...
- `boot.R` (4,934 lines), `plot.R` (5,312) and `default.R` (4,104) remain the largest files; the
