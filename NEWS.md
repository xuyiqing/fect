...
* `r` now defaults to `NULL`: for `method = "ife"` or `"gsynth"`, a call without `r` selects the number of factors by cross-validation over 0 to 5; before, it fitted no factors (details below).
* Calls that give the variables as strings (`Y = "y", D = "d"`) now use the same cross-validation defaults as formula calls (rolling folds, `nlambda = 10`), so they can select a different `r` or `lambda`; `cv.method = "block"` gives the old folds (details below).
* `seed` now fixes the cross-validation folds when `se = FALSE`; before, it had no effect there, so calls that set `seed` can select a different `r` or `lambda` (details below).
* With `se = TRUE`, cross-validation now uses `cv.rule`, `cv.buffer`, `cv.donut`, `min.T0` and `proportion` as given, and `method = "gsynth"` and `method = "ife"` with never-treated fitting now apply `cv.rule` (they always used `"1se"`). Calls that set these, and calls with `criterion = "moment"` and `se = TRUE`, can select a different `r` (#146; details below).
* `method = "cfe"` with `time.component.from = "nevertreated"` now applies `cv.rule`; with the default `"1se"` it can select a smaller `r`, and `cv.rule = "1pct"` usually gives the old choice.
* The equivalence (TOST) test for the leave-one-out pre-trend estimates (`loo = TRUE`) now uses the same pre-treatment periods as the F test, so its p-value can change.
* `estimand()` now reads each bootstrap replicate's own units. Before, its standard errors for the overall ATT, APTT, log-ATT and the placebo and carryover tests were wrong whenever the treated units did not come first in the data with one adoption time (3 to 6 times too small on `turnout`). `imputed_outcomes(replicates = TRUE)` had the same error (#150, xuyiqing/gsynth#37, xuyiqing/gsynth#60).
* `effect()`, `att.cumu()` and `estimand("att.cumu")` now use the variance type stored on the fit. With parametric inference, their confidence intervals were centered near zero when `vartype` was not typed as a string in the call, and in `att.cumu()` always (xuyiqing/gsynth#37, xuyiqing/gsynth#60, xuyiqing/gsynth#75).
...
* The bootstrap now resamples correctly from a group of one unit. With one treated unit (or one unit with treatment reversals), most draws used to be dropped without a message, so the standard error came from a few draws or was NA. With `vartype = "parametric"` and two never-treated units, the draws used the wrong units.
...
* A factor time index is now read in its level order, and a factor whose levels are increasing numbers, or a character index that holds numbers, gives the same fit as those numbers. Before, both were sorted as text ("1", "10", "2", ...), which scrambled the event times or made `method = "gsynth"` stop with a false message about treatment reversals (xuyiqing/gsynth#13).
* A covariate that is an exact linear combination of other covariates no longer changes the estimate: it is dropped with a warning and its coefficient is NA, as in `lm()`. Before, it could move the ATT, for example from 5.54 to 6.51 with `X3 = 2 * X1` on `simgsynth` (`method = "gsynth"`, `r = 2`), or stop the fit with "inv(): matrix is singular" (xuyiqing/gsynth#80, xuyiqing/gsynth#83).

## Input checks that now stop or warn

* Formula terms must be column names. `log(Y + 20) ~ D`, `factor(g)`, `X1 * X2` and `.` now stop with a message that says to create the column first; before, `log(Y + 20) ~ D` was fitted as `Y ~ D`, `factor(g)` as one linear term and `X1 * X2` as `X1 + X2`. An outcome that also appears on the right-hand side stops too. Intercept terms such as `+ 0` or `- 1` are still accepted and ignored. The same rules apply to `interFE()` (xuyiqing/gsynth#13, xuyiqing/gsynth#33).
* Covariates given both in the formula and in `X` now stop; `X` used to be ignored. `X` with a repeated name, or naming the outcome or the treatment, also stops (xuyiqing/gsynth#23).
* Non-numeric covariates (factor, character, Date) now stop with a message that says to create numeric dummy columns first; they used to stop with misleading errors. Logical covariates are used as 0/1, as before (xuyiqing/gsynth#33).
* A character time index must hold numbers; otherwise `fect()` stops and suggests a number, a Date, or a factor whose levels are in time order (xuyiqing/gsynth#13).
...
* Bootstrap runs with few never-treated units no longer stop with "number of items to replace ..."; a replicate that cannot be fitted is dropped. Whenever replicates are dropped, `fect()` says how many, and it warns when fewer than two remain. A main fit whose factors cannot be estimated stops with a plain message (xuyiqing/gsynth#32).
