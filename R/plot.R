...
plot.fect <- function(x,
  type = NULL, # gap, equiv, status, exit, factors, loadings, calendar, counterfactual,hte,gap.sub
  loo = FALSE,
  ...
  ...){
    ...
    if(!is.null(type)){
        ...
        if (!type %in% c("status", "gap","equiv","exit","factors","loadings","calendar","box","counterfactual","hte","gap.sub")) {
            stop("\"type\" option misspecified. Must be one of followings:\"status\",\"gap\",\"equiv\",\"exit\",\"calendar\",\"box\",\"counterfactual\",\"hte\",\"gap.sub\".")
        }
    ...
    if(type == "hte"){
      ...
      if (CI == FALSE) {
        ...
        d1 <- cbind.data.frame(YVAR,XVAR)

        if (moderator.loess == TRUE){
          YVAR.fit = rep(NA,length(x$avg.HTE.fit))
          XVAR.fit = rep(NA,length(x$avg.HTE.fit))
          for (i in 1:length(x$avg.HTE.fit)){
            YVAR.fit[i] = x$avg.HTE.fit[i]
            XVAR.fit[i] = x$Val.HTE.fit[i]
          }
          d2 <- cbind.data.frame(YVAR.fit,XVAR.fit)
        }
        ...
        d1 <- cbind.data.frame(YVAR,YMIN,YMAX,XVAR)

        if (moderator.loess == TRUE){
          YVAR.fit = rep(NA,length(x$avg.HTE.fit))
          XVAR.fit = rep(NA,length(x$avg.HTE.fit))
          YMIN.fit = rep(NA,length(x$avg.HTE.fit))
          YMAX.fit = rep(NA,length(x$avg.HTE.fit))
          for (i in 1:length(x$avg.HTE.fit)){
            YVAR.fit[i] = x$est.avg.HTE.fit[i,1]
            YMIN.fit[i] = x$est.avg.HTE.fit[i,3]
            YMAX.fit[i] = x$est.avg.HTE.fit[i,4]
            XVAR.fit[i] = x$Val.HTE[i]
          }
          d2 <- cbind.data.frame(YVAR.fit,YMIN.fit,YMAX.fit,XVAR.fit)
        }
      ...
      ## xlab and ylab
      p <- p + xlab(xlab) +  ylab(ylab)
      if(x$moderator.type == "continuous"){
      ...
      if(CI==FALSE){
        ...
        p <- p + geom_point(aes(x=d1[,2],y=d1[,1]),color='gray50',fill='gray50',alpha=1)
        if (moderator.loess == TRUE){
          p <- p + geom_line(aes(x=d2[,2],y=d2[,1]),color='skyblue',size=1.1)
        }
      } else {
        ...
        if (moderator.loess == TRUE){
          p <- p + geom_line(aes(x=d2[,4],y=d2[,1]),color='skyblue',size=1.1)
          p <- p + geom_ribbon(aes(x=d2[,4],ymin=d2[,2],ymax=d2[,3]),color='skyblue',fill='skyblue',alpha=0.5,size=0)
        }
      }
      if(count==TRUE){
    ...
    if(type == 'gap.sub'){
      ...
      #decide which value of moderator to display
      moderator.value = x$Val.HTE
      if (is.null(moderator)){
        cat("Please choose and assign a value to moderator from the following list:\n")
        print(moderator.value)
        stop("The program has been stopped because moderator is NULL.")
      }
      else{
        if (! moderator %in% moderator.value){
          cat("Please choose and assign a value to moderator from the following list:\n")
          print(moderator.value)
          stop("The program has been stopped because of wrong value of moderator.")
        }
      }

      MODid = which(moderator.value == moderator)
      ...
      if (CI == FALSE) {
        message("Uncertainty estimates not available.\n")
        temp.att <- x$att.HTE[[MODid]]
        temp.time <- x$time.HTE[[MODid]]
        temp.count <- x$count.HTE[[MODid]]
        ...
        temp.est.att = x$est.att.HTE[[MODid]]
        temp.time <- x$time.HTE[[MODid]]
        rm.pos <- which(is.na(temp.est.att[,3]))
        if (length(rm.pos) != 0){
          temp.est.att <- temp.est.att[-rm.pos,]
          temp.time <- x$time.HTE[[MODid]][-rm.pos]
        }
      ...
      ## title
      if (is.null(main) == TRUE) {
        temp.title = paste0("Dynamic Effect when Moderator = ",x$Val.HTE[MODid])
