...
fect_boot <- function(
  ...
  if (binary == TRUE & vartype == "parametric") {
    ...
    if (isTRUE(split_residuals)) {
      ...
      ## every time period with at least 2 controls.  cov >= 2 (not >= 1)
      ## is required because draw.error excludes fake.tr from id.co_A
      ## downstream; with cov_A == 1 for some year t, if fake.tr happens
      ## to be that single covering control, id.co_A_rest drops year t to
      ## zero coverage and the inner repeat loop in draw.error hangs.
      ## Empirical triggers: EH 2023 sec_enrol on variant (iii) parametric
      ## bootstrap (TT=63, 27% missing).  Fall back to no-split (full
      ## id.co) after a bounded number of redraw attempts.
      ...
      if (!.partition_ok) {
        warning(sprintf(
          "split_residuals: could not partition %d controls into two halves with cov_A >= 2 and cov_B >= 2 per year in %d tries; falling back to no-split (using full id.co for both halves).",
    ...
    one.nonpara <- function(num = NULL) {
      ...
      if (para.error.resolved == "ar") {
        ...
        ## NOTE (merge 2026-08-12): split_residuals has no branch here -- "wild" did not
        ## exist when split_residuals was designed, so this path always uses the full
        ## control pool (error.co / Nco) regardless of split_residuals. Flagged for the
        ## user; not resolved as part of this merge (see report).
