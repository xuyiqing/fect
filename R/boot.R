...
fect.boot <- function(Y,
    ...
    eff.boot <- array(NA,dim = c(TT, N, nboots))  ## to store results
    ...
    for (i in c(1:length(out$att.HTE))){
      att.boot.HTE[[i]] = matrix(0,length(out$time.HTE[[i]]),nboots)
    }
    HTE.eff.boot <- matrix(0,length(N.HTE),nboots)
    HTE.eff.fit.boot <- matrix(0,length(N.HTE),nboots)
    # HTEcoef.boot <- matrix(0,1,nboots)
    }

    # att.HTE.boot <- list()
    # for (i in c(1:length(att.HTE))){
    #   att.HTE.boot[[i]] <- matrix(0,length(time.HTE[[i]]),nboots)
    # }
    ...
    if (parallel == TRUE) {
        ...
        for (j in 1:nboots) {
            ...
            # for (i in c(1:length(att.HTE))){
            #   time.temp <- c(boot.out[[j]]$time.HTE[[i]])
            #   index.temp <- which(c(time.HTE[[i]]) %in% time.temp)
            #   att.HTE.boot[[i]][index.temp,j] <- boot.out[[j]]$att.HTE[index.temp]
            # }
            ...
            for (k in c(1:length(att.boot.HTE))){
              ...
              temp.time.HTE <- boot.out[[j]]$time.HTE[[k]]
        ...
        for (j in 1:nboots) {
            ...
            #   index.temp <- which(c(time.HTE[[i]]) %in% time.temp)
            #   att.HTE.boot[[i]][index.temp,j] <- boot$att.HTE[index.temp]
            # }
    ...
    if (sum(is.na(c(att.avg.boot))) > 0) {
        ...
        # for (i in c(1:length(att.HTE))){
        #   att.HTE.boot[[i]] <- as.matrix(att.HTE.boot[[i]][,-boot.rm])
        # }
    ...
    if (vartype == "jackknife") {
        ...
        if(length(HTEid) == 1){
        avg.HTE.j <- jackknifed(avg.HTE,avg.HTE.boot,alpha,quantile.CI = quantile.CI)
        est.avg.HTE <- cbind(avg.HTE, avg.HTE.j$se, avg.HTE.j$CI.l, avg.HTE.j$CI.u, avg.HTE.j$P, out$N.HTE)
        colnames(est.avg.HTE) <- c("AVG", "S.E.", "CI.lower", "CI.upper",
                                   "p.value", "count")
        rownames(est.avg.HTE) <- out$Val.HTE
        est.att.HTE <- list()
        for (i in c(1:length(att.boot.HTE))){
          temp.att.j <- jackknifed(att, att.boot.HTE[[i]], alpha, quantile.CI = quantile.CI)
          est.att.HTE[[i]] <- cbind(out$att.HTE[[i]],temp.att.j$se,temp.att.j$CI.l,temp.att.j$CI.u,temp.att.j$P,out$count.HTE[[i]])
          colnames(est.att.HTE[[i]]) <- c("ATT", "S.E.", "CI.lower", "CI.upper",
                                         "p.value", "count")
        ...
        if(length(HTEid) == 1){
        se.avg.HTE <- apply(avg.HTE.boot, 1, function(vec) sd(vec, na.rm=TRUE))
        if(quantile.CI == FALSE){
          CI.avg.HTE <- cbind(avg.HTE - se.avg.HTE * qnorm(1-alpha/2), avg.HTE + se.avg.HTE * qnorm(1-alpha/2)) # normal approximation
          pvalue.avg.HTE <- (1-pnorm(abs(avg.HTE/se.avg.HTE)))*2
        }
        else{
          CI.avg.HTE <- t(apply(avg.HTE.boot, 1, function(vec) quantile(vec,c(alpha/2, 1 - alpha/2), na.rm=TRUE)))
          pvalue.avg.HTE <- apply(avg.HTE.boot, 1, get.pvalue)
        }

        est.avg.HTE <- cbind(avg.HTE, se.avg.HTE, CI.avg.HTE, pvalue.avg.HTE, out$N.HTE)
        colnames(est.avg.HTE) <- c("ATT", "S.E.", "CI.lower", "CI.upper",
                                   "p.value", "count")
        rownames(est.avg.HTE) <- out$Val.HTE
        ...
        for (i in c(1:length(out$att.HTE))){
          temp.se.att <- apply(att.boot.HTE[[i]], 1, function(vec) sd(vec, na.rm=TRUE))
          if(quantile.CI == FALSE){
            temp.CI.att <- cbind(out$att.HTE[[i]] - temp.se.att * qnorm(1-alpha/2), out$att.HTE[[i]] + temp.se.att * qnorm(1-alpha/2)) # normal approximation
            temp.pvalue.att <- (1-pnorm(abs(out$att.HTE[[i]]/temp.se.att)))*2
          }
          else{
            temp.CI.att <- t(apply(att.boot.HTE[[i]], 1, function(vec) quantile(vec,c(alpha/2, 1 - alpha/2), na.rm=TRUE)))
            temp.pvalue.att <- apply(att.boot.HTE[[i]], 1, get.pvalue)
          }
        ...
        # if(length(HTEid) == 1){
        # se.eff.HTE <- apply(HTE.eff.boot, 1, function(vec) sd(vec, na.rm=TRUE))
        # if(quantile.CI == FALSE){
        #     CI.eff.HTE <- cbind(HTE.eff - se.eff.HTE * qnorm(1-alpha/2), HTE.eff + se.eff.HTE * qnorm(1-alpha/2))
        #     pvalue.eff.HTE <- (1-pnorm(abs(HTE.eff/se.eff.HTE)))*2
        # }
        # else{
        #     CI.eff.HTE <- t(apply(HTE.eff.boot, 1, function(vec) quantile(vec,c(alpha/2, 1 - alpha/2), na.rm=TRUE)))
        #     pvalue.eff.HTE <- apply(HTE.eff.boot, 1, get.pvalue)
        # }
        ...
        #     pvalue.eff.HTE.fit <- (1-pnorm(abs(HTE.eff.fit/se.eff.HTE.fit)))*2
        # }
        # else{
        #     CI.eff.HTE.fit <- t(apply(HTE.eff.fit.boot, 1, function(vec) quantile(vec,c(alpha/2, 1 - alpha/2), na.rm=TRUE)))
        #     pvalue.eff.HTE.fit <- apply(HTE.eff.fit.boot, 1, get.pvalue)
        # }
        # est.eff.HTE.fit <- cbind(HTE.eff.fit, se.eff.HTE.fit, CI.eff.HTE.fit, pvalue.eff.HTE.fit,HTE.N)
        # colnames(est.eff.HTE.fit) <- c("ATT-HTE Fitted", "S.E.", "CI.lower", "CI.upper","p.value", "count")
        # }
        # else{
        #   est.eff.HTE.fit <- NULL
        # }
        # # se.HTEcoef <- apply(HTEcoef.boot, 1, function(vec) sd(vec, na.rm=TRUE))
        # # CI.HTEcoef <- cbind(HTEcoef - se.HTEcoef * qnorm(1-alpha/2), HTEcoef + se.HTEcoef * qnorm(1-alpha/2))
        # # pvalue.HTEcoef <- (1-pnorm(abs(HTEcoef/se.HTEcoef)))*2
        # # est.HTEcoef <- cbind(HTEcoef,se.HTEcoef,CI.HTEcoef,pvalue.HTEcoef)
        # # colnames(est.HTEcoef) <- c("HTE between Groups", "S.E.", "CI.lower", "CI.upper","p.value")
        # }
    ...
    ##storage
    #NULL result if HTEvar not exist
    if (length(HTEid) != 1){
      est.avg.HTE = NULL
      avg.HTE = NULL
      est.avg.HTE.fit = NULL
