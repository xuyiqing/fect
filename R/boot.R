...
## Resample the elements of `x`. sample(x, ...) treats a length-1 numeric x
## as 1:x, so with one treated unit (or one control, one reversal unit, ...)
## it drew unit ids that are not in x. Indexing through sample.int() always
## draws from x itself; for length(x) > 1 it makes exactly the draws
## sample(x, ...) makes, so every other fit is unchanged.
...
## Is a bootstrap / jackknife refit usable by the collectors? A refit that
## returned without error can still be degenerate (e.g. an early return
## with missing fields); the collectors assign att.avg, att.avg.unit and att
## into fixed-size slots and, with keep.sims, eff into a TT-row slice.
## Anything else is treated as a failed replicate (dropped and counted).
.fect_boot_result_ok <- function(res, n.time.on, TT, keep.sims = FALSE,
                                 n.col = NULL) {
  if (!is.list(res)) return(FALSE)
  if (length(res$att.avg) != 1L || length(res$att.avg.unit) != 1L) {
    return(FALSE)
  }
  if (length(res$att) != n.time.on) return(FALSE)
  if (isTRUE(keep.sims)) {
    if (!is.matrix(res$eff) || nrow(res$eff) != TT) return(FALSE)
    if (!is.null(n.col) && ncol(res$eff) != n.col) return(FALSE)
  }
  TRUE
}
...
fect_boot <- function(
  ...
  if (binary == TRUE & vartype == "parametric") {
    one.nonpara <- function(num = NULL) {
      ...
      ## a refit that returned but is degenerate counts as failed
    ...
    if (Nco < 2) {
      stop(
        ...
        "pseudo-treated unit and the others as controls. ",
    ...
    draw.error <- function() {
      ...
      synth.out <- try(
        impute_Y0(
          method     = method,
          predictive = time.component.from,
          Y          = Y.pseudo,
          X          = X.pseudo,
          D          = D.pseudo,
          W          = NULL,
    ...
    one.nonpara <- function(num = NULL) {
      ...
      ## Weights follow the replicate's units (treated first, then the drawn
      ## controls), so each replicate returns its weighted aggregates.
      ...
      ## Extra-FE slicing for cfe branches (new for Phase 3b)
      X.extra.FE.boot <- if (!is.null(X.extra.FE) && length(dim(X.extra.FE)) == 3 && dim(X.extra.FE)[2] > 0) {
          X.extra.FE[, id.boot, , drop = FALSE]
      } else {
          X.extra.FE
      }
      X.Z.boot <- if (!is.null(X.Z) && length(dim(X.Z)) == 3 && dim(X.Z)[2] > 0) {
          X.Z[, id.boot, , drop = FALSE]
      } else {
          X.Z
      }
      X.Q.boot <- if (!is.null(X.Q) && length(dim(X.Q)) == 3 && dim(X.Q)[2] > 0) {
          X.Q[, id.boot, , drop = FALSE]
      } else {
          X.Q
      }
      X.gamma.boot <- if (!is.null(X.gamma) && length(dim(X.gamma)) == 3 && dim(X.gamma)[2] > 0) {
          X.gamma[, id.boot, , drop = FALSE]
      } else {
          X.gamma
      }
      X.kappa.boot <- if (!is.null(X.kappa) && length(dim(X.kappa)) == 3 && dim(X.kappa)[2] > 0) {
          X.kappa[, id.boot, , drop = FALSE]
      } else {
          X.kappa
      }
  ...
  if (do_parallel_boot) {
    ...
    for (j in 1:nboots) {
      ...
      ## Collector guard: a replicate whose fields do not fit the collector
      ## slots is recorded with the fields of make_boot_na() (as a failed
      ## worker is), so boot.rm drops it and it is counted, instead of the
      ## run aborting. A second failure would be an internal error: re-raise.
      for (collect.try in 1:2) {
        collect.ok <- tryCatch({
          ...
          if (hasRevs == 1) {
            ...
            if (keep.sims) {
              eff.off.boot[j] <- list(boot.out[[j]]$eff.off)
            }
          }
          if (!is.null(T.on.carry)) {
    ...
    for (j in 1:nboots) {
      ...
      boot.out[[j]] <- boot
      ## Collector guard (same as the parallel loop): a replicate whose
      ## fields do not fit the collector slots is recorded with the fields
      ## of make_boot_na(), so boot.rm drops it and it is counted.
      for (collect.try in 1:2) {
        collect.ok <- tryCatch({
  ...
  n.fail <- 0L  ## replicates dropped below (failed or unusable refits)
  ...
  ## Say how many replicates were dropped, instead of dropping them silently.
  if (n.fail > 0L) {
