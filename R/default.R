...
fect.default <- function(
    ...
    ## covariates must be numeric. A logical column is used as 0/1, as it
    ## already was implicitly. Before 2.4.6 a factor stopped with R's
    ## "var(x) on a factor" error and a character column with a false
    ...
    for (x.name in X) {
        ...
        if (is.logical(x.col)) {
            ...
            stop(
                ...
                "numeric covariates. Create numeric (dummy) columns first, ",
                "for example with `model.matrix(~ ", x.name, ", data)`, and ",
                "use those columns.",
                call. = FALSE
            )
        }
    ...
    ## effect(), att.cumu() and estimand() read the variance type actually
    ## used from the fit, not from the call (the call holds NULL when
    ## vartype was not typed, or a symbol when it was passed as a variable).
    ## fect_boot() stores it; make sure every se = TRUE fit carries it.
