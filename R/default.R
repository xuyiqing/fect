...
fect.default <- function(
    ...
    ## Parametric bootstrap requires nevertreated control-pool isolation, UNLESS
    ## split_residuals = TRUE and time.component.from = "notyettreated" (branch-only
    ## "Gate C": split_residuals's K=2 cross-fit isolates the control pool a
    ## different way, so the nevertreated restriction can be relaxed for it).
    ...
    } else if (se == 1 && vartype == "parametric" && time.component.from != "nevertreated") {
        stop(sprintf(
            paste0(
                ...
                "exist), use vartype = \"bootstrap\" or \"jackknife\", or (when ",
                "time.component.from = \"notyettreated\") set split_residuals = TRUE ",
                "to use the experimental debiased parametric bootstrap."
            ),
