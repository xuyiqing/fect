...
fect_cfe <- function(
    ...
    if (binary == FALSE && boot == FALSE) {
        ...
        pre.sd <- cbind(
            pre.sd,
            sort(unique(eff.pre.equiv[, 2])),
            table(eff.pre.equiv[, 2])
        )
    ...
    if (!is.null(W)) {
        ...
        att.on.sum.W <- as.numeric(tapply(
            eff.v.use.W * W.v.use,
            t.on.use.W,
            sum
        )) ## NA already removed
        ...
        if (!is.null(time.on.seq.W)) {
            att.on.sum.med.W <- W.on.sum.med <- count.on.med.W <- att.on.med.W <- rep(
                NA,
                length(time.on.seq.W)
            )
            att.on.sum.med.W[which(
                time.on.seq.W %in% time.on.W
            )] <- att.on.sum.W
    ...
    if (!is.null(balance.period)) {
        ...
        if (!is.null(time.on.balance.seq)) {
            ...
            balance.att.med[which(
                time.on.balance.seq %in% balance.time
            )] <- balance.att
            if (length(balance.count) > 0) {
                balance.count.med[which(
                    time.on.balance.seq %in% balance.time
                )] <- balance.count
            }
        ...
        if (!is.null(placebo.period) && placeboTest == 1) {
            if (length(placebo.period) == 1) {
                ...
                balance.placebo.pos <- which(
                    balance.time >= placebo.period[1] &
                        balance.time <= placebo.period[2]
                )
                balance.att.placebo <- sum(
                    balance.att[balance.placebo.pos] *
                        balance.count[balance.placebo.pos]
                ) /
                    sum(balance.count[balance.placebo.pos])
            }
        }
    }
    ...
    if (!is.null(placebo.period) && placeboTest == 1) {
        if (length(placebo.period) == 1) {
            ...
            placebo.pos <- which(
                time.on >= placebo.period[1] & time.on <= placebo.period[2]
            )
            att.placebo <- sum(att.on[placebo.pos] * count.on[placebo.pos]) /
                sum(count.on[placebo.pos])
        }
        ...
        if (!is.null(W)) {
            if (length(placebo.period) == 1) {
                ...
                placebo.pos.W <- which(
                    time.on.W >= placebo.period[1] &
                        time.on.W <= placebo.period[2]
                )
                att.placebo.W <- sum(att.on.sum.W[placebo.pos.W]) /
                    sum(W.on.sum[placebo.pos.W])
            }
        }
    }
    ...
    if (hasRevs == 1) {
        ...
        if (binary == FALSE && boot == FALSE) {
            ...
            off.sd <- cbind(
                off.sd,
                sort(unique(eff.off.equiv[, 2])),
                table(eff.off.equiv[, 2])
            )
        ...
        if (!is.null(W)) {
            ...
            att.off.sum.W <- as.numeric(tapply(
                eff.v.use2.W * W.v.use2,
                t.off.use.W,
                sum
            ))
            ...
            if (!is.null(time.off.seq.W)) {
                att.off.sum.med.W <- W.off.sum.med <- count.off.med.W <- att.off.med.W <- rep(
                    NA,
                    length(time.off.seq.W)
                )
                att.off.sum.med.W[which(
                    time.off.seq.W %in% time.off.W
                )] <- att.off.sum.W
                att.off.med.W[which(
                    time.off.seq.W %in% time.off.W
                )] <- att.off.W
                count.off.med.W[which(
                    time.off.seq.W %in% time.off.W
                )] <- count.off.W
                W.off.sum.med[which(
                    time.off.seq.W %in% time.off.W
    ...
    if (!is.null(carryover.period) && carryoverTest == 1 && hasRevs == 1) {
        ...
        if (length(carryover.period) == 1) {
            ...
            carryover.pos <- which(
                time.off >= carryover.period[1] &
                    time.off <= carryover.period[2]
            )
            att.carryover <- sum(
                att.off[carryover.pos] * count.off[carryover.pos]
            ) /
                sum(count.off[carryover.pos])
        }
        ...
        if (!is.null(W)) {
            if (length(carryover.period) == 1) {
                ...
                carryover.pos.W <- which(
                    time.off.W >= carryover.period[1] &
                        time.off.W <= carryover.period[2]
                )
                att.carryover.W <- sum(att.off.sum.W[carryover.pos.W]) /
                    sum(W.off.sum[carryover.pos.W])
            }
        }
    }
    ...
    if (sum(!is.na(eff.calendar)) > 1) {
        ...
        if (is.null(calendar.enp.seq)) {
            loess.fit <- suppressWarnings(try(
                loess(eff.calendar ~ T.calendar, weights = N.calendar),
                silent = TRUE
            ))
        } else {
            loess.fit <- suppressWarnings(try(
                loess(
                    eff.calendar ~ T.calendar,
                    weights = N.calendar,
                    enp.target = calendar.enp.seq
                ),
                silent = TRUE
            ))
        }
    ...
    if (!is.null(group)) {
        ...
        for (i in c(1:length(group.level))) {
            ...
            if (!is.null(time.on.seq.group)) {
                count.on.med.sub <- att.on.med.sub <- rep(
                    NA,
                    length(time.on.seq.group[[sub.group.name]])
                )
                time.on.seq.sub <- time.on.seq.group[[sub.group.name]]
                att.on.med.sub[which(
                    time.on.seq.sub %in% time.on.sub
                )] <- att.on.sub
                count.on.med.sub[which(
                    time.on.seq.sub %in% time.on.sub
                )] <- count.on.sub
            ...
            if (!is.null(placebo.period) && placeboTest == 1) {
                if (length(placebo.period) == 1) {
                    ...
                    placebo.pos.sub <- which(
                        time.on.sub >= placebo.period[1] &
                            time.on.sub <= placebo.period[2]
                    )
                    if (length(placebo.pos.sub) > 0) {
                        att.placebo.sub <- sum(
                            att.on.sub[placebo.pos.sub] *
                                count.on.sub[placebo.pos.sub]
                        ) /
                            sum(count.on.sub[placebo.pos.sub])
                    } else {
                        att.placebo.sub <- NULL
                    }
                }
            ...
            if (hasRevs == 1) {
                ...
                if (!is.null(time.off.seq.group)) {
                    count.off.med.sub <- att.off.med.sub <- rep(
                        NA,
                        length(time.off.seq.group[[sub.group.name]])
                    )
                    time.off.seq.sub <- time.off.seq.group[[sub.group.name]]
                    att.off.med.sub[which(
                        time.off.seq.sub %in% time.off.sub
                    )] <- att.off.sub
                    count.off.med.sub[which(
                        time.off.seq.sub %in% time.off.sub
                    )] <- count.off.sub
                ...
                if (!is.null(carryover.period) && carryoverTest == 1) {
                    if (length(carryover.period) == 1) {
                        carryover.pos.sub <- which(
                            time.off.sub == carryover.period
                        )
                        ...
                        carryover.pos.sub <- which(
                            time.off.sub >= carryover.period[1] &
                                time.off.sub <= carryover.period[2]
                        )
                        if (length(carryover.pos.sub) > 0) {
                            att.carryover.sub <- sum(
                                att.off.sub[carryover.pos.sub] *
                                    count.off.sub[carryover.pos.sub]
                            ) /
                                sum(count.off.sub[carryover.pos.sub])
                        } else {
                            att.carryover.sub <- NULL
                        }
                    }
                    ...
                    suboutput <- c(
                        suboutput,
                        list(att.carryover = att.carryover.sub)
                    )
                }
            }
    ...
    if (!is.null(balance.period)) {
        out <- c(
            out,
            list(
                balance.att = balance.att,
                balance.time = balance.time,
                balance.count = balance.count,
                balance.avg.att = att.avg.balance
            )
        )
        if (!is.null(placebo.period) && placeboTest == 1) {
