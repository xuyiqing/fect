...
effect <- function(x, ## a fect object
  ...
  # single selected unit as a TT x 1 matrix)
  ...
  # Inference type: the variance type fect resolved and stored on the fit.
  # The call is only a fallback (older fits): it holds NULL when vartype was
  # not typed (e.g. gsynth's default parametric inference) and a symbol when
  # it was passed as a variable.
