...
A `fect()` fit with `se = TRUE` and `keep.sims = TRUE` stores bootstrap estimates in `fit$eff.boot`, a $T \times N \times B$ array. Slice $b$ holds the units of replicate $b$ in the order `fit$colnames.boot[[b]]` (their column positions in the original panel): under the unit bootstrap a unit drawn twice appears twice, and under the parametric bootstrap the treated units come first. With a cluster bootstrap (`cl`), replicates differ in size and the array is padded with `NA` to the widest one. For `vartype = "jackknife"`, it stores a $T \times (N-1) \times N$ array instead. Two functions use this array:
...
- **`estimand()`** is used after fitting. It aggregates `fit$eff.boot` into a one-dimensional bootstrap distribution for the requested estimand: `att`, `att.cumu`, `aptt`, or `log.att`. It then builds a CI using the selected `ci.method` (the cumulative ATT is an exception; see below). Five values are accepted: `"normal"`, `"basic"`, `"percentile"`, `"bc"`, and `"bca"`.

The two paths give the same results where they overlap. `estimand(fit, "att", "event.time")` returns `fit$est.att` as it is, and the overall ATT with `"normal"` or `"basic"` CIs matches `fit$est.avg` up to rounding error. The additional methods, `"percentile"`, `"bc"`, and `"bca"`, are available only through `estimand()`. This is also where the estimand-specific defaults are defined: `aptt → "bca"` and `log.att → "bca"`.
...
The default `ci.method` depends on the estimand: `att` → `"normal"`, `aptt` → `"bca"`, and `log.att` → `"bca"`. The cumulative ATT does not use `ci.method` (see below). These defaults follow the meaning of each estimand, not performance on any specific DGP. See the coverage results below for the full comparison.
...
**Caveat.** The simulation is run under $H_0$, with no treatment effect added back to treated cells. As a result, `fit$eff.boot` is centered at zero. The legacy `fit$est.avg` output and `get.pvalue()` use this $H_0$ centering to test $H_0: \mathrm{ATT} = 0$. In the current implementation, `estimand()` applies a post-hoc location shift for parametric fits: `att_b <- att_b - mean(att_b) + estimate`. This centers the bootstrap distribution at the point estimate while preserving its spread, since `sd()` is unchanged by a shift. The `"normal"` CI is unchanged by this fix; the other four CI methods now produce correctly centered CIs. The legacy cumulative-effect functions handle parametric fits separately: `effect()` and `att.cumu()` report the estimate plus or minus the normal critical value times the SE, with the matching $p$-value. Both read the variance type stored on the fit (`fit$vartype`); before v2.4.7, `att.cumu()`, and `effect()` when `vartype` was not typed as a string in the call, took quantiles of the $H_0$-centered draws, giving intervals centered near zero.
...
Two further rules apply. The parametric bootstrap needs at least two never-treated units, because each simulated error uses one control unit as a pseudo-treated unit and the others as controls; with fewer, `fect()` stops. It also ignores `cl`, with a warning: it simulates errors unit by unit and does not resample clusters, so its SEs do not account for within-cluster correlation. Use `vartype = "bootstrap"` with `cl` for cluster-aware inference.
...
fect(data = data, Y = "Y", D = "D", index = c("id", "time"),
...
```
The `"basic"` interval is the reflected (pivot) CI of @davison_hinkley1997 §5.2.1. It is the standard "percentile" CI in the literature and is what `boot::boot.ci(type = "basic")` returns in R. The default is `"normal"`. The `est.att` (per-event-time) and `est.avg` (overall) slots returned by `fect()` use the requested method. On the same fit, `estimand(fit, "att", "event.time")` returns `est.att` as it is, and `estimand(fit, "att", "overall", ci.method = ...)` with the same method matches `est.avg` up to rounding error.
...
**Other slots.** The location-shift fix is applied only to `est.att` and `est.avg`. If your workflow uses other `fect()` slots, such as `est.calendar`, `est.cohort.att`, `est.subgroup.att`, balanced-sample, by-W, placebo, or carryover, and you use `vartype = "parametric"` with `ci.method = "basic"`, those slots still use the legacy reflected formula on the raw $H_0$-centered bootstrap. Their CIs may not be correctly centered. Until these slots are updated, get the placebo and carryover series from `estimand(fit, "att", "event.time", test = "placebo", ci.method = "basic")` (or `test = "carryover"`). `estimand()` has no counterpart yet for the calendar-time, cohort, subgroup, balanced-sample and by-W slots.
...
| `"basic"` | $(2\hat\theta - q_{1-\alpha/2},\ 2\hat\theta - q_{\alpha/2})$ | Reflected pivot CI [@davison_hinkley1997 §5.2.1; `boot::boot.ci(type = "basic")`] |
...
In `estimand()`, `ci.method = NULL` uses an estimand-specific default: `att` → `"normal"`, `aptt` → `"bca"`, and `log.att` → `"bca"`. Pass an explicit value to override the default. The per-event-time ATT is the exception: `estimand(fit, "att", "event.time")` returns `fit$est.att` as a data frame, and any other `ci.method`, `conf.level` or `window` value stops with an error. Use `by = "overall"` for the other methods.

The cumulative ATT does not use `ci.method` or `conf.level`. `estimand(fit, "att.cumu", ...)` returns the 95% intervals of `effect()` (by event time) or `att.cumu()` (over one window). For bootstrap fits these are percentile intervals: the 2.5% and 97.5% quantiles of the replicates' cumulative ATTs. For parametric fits they are symmetric intervals around the estimate, built from the bootstrap standard error. For jackknife fits they are the estimate plus or minus the normal critical value times the jackknife standard error. Internally, `ci.method = NULL` still becomes `"basic"` for `att.cumu`. So `estimand()` warns about tail quantiles when `nboots` is below 1000, and a jackknife fit needs `ci.method = "normal"`; with the default it stops.
...
`fect(ci.method = "normal")` and `estimand(fit, type = "att", ci.method = "normal")` give the same per-event-time estimates and intervals. For the overall ATT, which needs `keep.sims = TRUE`, `estimand(fit, "att", "overall", ci.method = ...)` matches `fit$est.avg` for `"normal"`, and for `"basic"` when the fit also used `ci.method = "basic"`. They agree up to rounding error (differences of about $10^{-15}$) when they use the same `fit$eff.boot`; two separate fits differ by bootstrap noise.

The two interfaces serve different purposes. `fect()` fills the `est.*` slots at fit time, so users get a CI in the printed output without calling `estimand()` separately. The two options, `"normal"` and `"basic"`, cover the routine `att` workflow. `estimand()` provides the full five-method set for the overall ATT, `aptt` and `log.att`, where the bootstrap distribution can be skewed and bias correction can change the CI.

To use `bca` for the overall ATT, fit the model with `keep.sims = TRUE` and then call `estimand(fit, "att", "overall", ci.method = "bca")`. In this release the per-event-time series has only the interval stored in `fit$est.att`. `fect()` rejects `ci.method = "bca"`, `"bc"`, and `"percentile"` with an error that points users to `estimand()`.
...
The legacy `fit$est.avg` $p$-value, stored in the `p.value` column of `fit$est.att`, uses the `"normal"` formula by default. With `ci.method = "basic"` (the legacy `quantile.CI = TRUE`), it uses the `"percentile"` formula for bootstrap fits, and for parametric fits the formula below. The current `estimand()` API does not yet return a `p.value` column. Users who need a non-`"normal"` $p$-value can compute it from `fit$eff.boot` using the formulas above. A `p.value` column is planned for a future release.

For `vartype = "parametric"`, the draws are simulated with no effect, so they are centered at zero. With `ci.method = "basic"` the $p$-value is twice the smaller share of the centered draws $B - \bar B$ ($\bar B$ is the mean of the draws) at or beyond the estimate, $2 \min(P(B - \bar B \ge \hat\theta),\ P(B - \bar B \le \hat\theta))$. This is the `"basic"` row of the table applied to the shifted draws $B - \bar B + \hat\theta$ that the interval uses, so it is below $\alpha$ when the basic interval excludes zero, up to the resolution of the draws. Before v2.4.7 the zero-centered draws were compared with zero, as if they were centered at the estimate, which gave $p$-values near 1 whatever the estimate.
