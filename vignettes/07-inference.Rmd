...
**Caveat.** The simulation is run under $H_0$, with no treatment effect added back to treated cells. As a result, `fit$eff.boot` is centered at zero. The legacy `fit$est.avg` output and `get.pvalue()` use this $H_0$ centering to test $H_0: \mathrm{ATT} = 0$. In the current implementation, `estimand()` applies a post-hoc location shift for parametric fits: `att_b <- att_b - mean(att_b) + estimate`. This centers the bootstrap distribution at the point estimate while preserving its spread, since `sd()` is unchanged by a shift. The `"normal"` CI is unchanged by this fix; the other four CI methods now produce correctly centered CIs. The legacy cumulative-effect functions handle parametric fits separately: `effect()` reports the estimate plus or minus a $t$ critical value times the SE, and `att.cumu()` the estimate plus or minus a normal critical value times the SE, each with the matching $p$-value. Both read the variance type stored on the fit (`fit$vartype`); before v2.4.6, `att.cumu()`, and `effect()` when `vartype` was not typed as a string in the call, took quantiles of the $H_0$-centered draws, giving intervals centered near zero.
