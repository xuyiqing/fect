...
To use `bca` for the overall ATT, fit the model with `keep.sims = TRUE` and then call `estimand(fit, "att", "overall", ci.method = "bca")`. The per-event-time series has only the normal interval in this release. `fect()` rejects `ci.method = "bca"`, `"bc"`, and `"percentile"` with an error that points users to `estimand()`.
