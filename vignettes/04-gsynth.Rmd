...
If we want to zoom in to examine the treatment effect on a specific state, such as Wisconsin, we can do so by setting the `id` option accordingly. The `id` option can also accept a vector if one wishes to plot multiple units. This plot can only be done if `keep.sims = TRUE` is set in the estimation step.
