...
### Key Equivalence
`fect(..., method = "gsynth")` is equivalent to `fect(..., method = "ife", factors.from = "nevertreated", em = FALSE)`.

Gsynth is not a separate estimator — it is the IFE model estimated using only never-treated controls, without the EM algorithm. The **gsynth** package was developed in the synthetic control research setting, where a small number of treated units are compared against never-treated controls to learn latent time effects.
:::
