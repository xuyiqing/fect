...
-   **Lines and Bounds:**
    ...
    -   `lcolor` / `lwidth` / `ltype`: Set the color, width, and linetype for the axes. Takes a vector, where the first value is applied the horizontal axis and the second is applied the vertical axis. If only one value is given, both axes will take on the same value.
...
plot(out, 
     ...
     est.lwidth = 1.5, # Makes the lines thicker
...
plot(out, type = "counterfactual",
     main = "Observed vs. Counterfactual Trajectory",
     ylab = "Outcome",
     legend.pos = "bottom")
```
...
Many adjustments can be made to the counterfactual plot with the same options as in the gap plot, as well as the `counterfactual.color`, `counterfactual.raw.controls.color`, `counterfactual.raw.treated.color`, and `counterfactual.linetype` options.
...
gs2020_no_reversals <- gs2020[gs2020[[unit]] %in% good_units, ]
...
out_no_reversals <- fect(Y = y, D = d, X = controls, index = c(unit, time), 
               data = gs2020_no_reversals, method = "fe", force = "two-way", 
               se = TRUE, parallel = TRUE, nboots = 100, keep.sims = TRUE)
```

Finally, we will plot the cumulative effects.

```{r cumulative-effects}
plot(effect(out_no_reversals))
```
...
plot(out_fe_carryover, type = "status",
     status.treat.color = "red", # Color for treated units
     status.control.color = "blue",   # Color for control units
     status.missing.color = "darkgreen",   # Color for missing data
     status.background.color = "beige", # Background color
     main = "Status Plot")
```
