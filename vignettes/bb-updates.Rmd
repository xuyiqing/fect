...
**New `vartype = "wild"`.** `fect()` now supports unit-level
Rademacher wild bootstrap (Liu 1988; Mammen 1993; cluster-robust
extension by Cameron, Gelbach, and Miller 2008). Wild keeps the unit
composition fixed and perturbs main-fit residuals on observed
control cells (`D = 0`, `I = 1`) by ±1 signs drawn once per unit.
Compared to case bootstrap, wild preserves the treated/control
composition exactly (no unit dropout) and respects within-unit
dependence; compared to parametric, it is fully nonparametric.
Supported for continuous outcomes with `method ∈ {fe, ife, mc,
gsynth, cfe}`; rejected for binary. Storage layout matches
`vartype = "bootstrap"`; `estimand()` consumes wild fits
transparently.
