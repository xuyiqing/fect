...
* **Parametric intervals**: `effect()`, `att.cumu()` and `estimand("att.cumu")` use the variance type stored on the fit. Their intervals were centered near zero when `vartype` was not typed as a string, and in `att.cumu()` always.
...
* **Cross-validation**: `criterion = "pc"` picks the lowest PC for never-treated fits ([gsynth #24](https://github.com/xuyiqing/gsynth/issues/24)). With `CV = TRUE`, `W.agg` alone no longer weights never-treated fits ([gsynth #101](https://github.com/xuyiqing/gsynth/issues/101)). With `parallel = FALSE`, `se = TRUE` draws the same folds as `se = FALSE`.
...
* **Time index and covariates**: a factor time index is read in its level order ([gsynth #13](https://github.com/xuyiqing/gsynth/issues/13)). An exactly collinear covariate is dropped with a warning, with coefficient NA, instead of changing the estimate ([gsynth #83](https://github.com/xuyiqing/gsynth/issues/83)).

**Input checks that now stop or warn**

* Formula terms must be column names: `log(Y)`, `factor(g)`, `X1 * X2` and `.` stop with a message to create the column first. Intercept terms are ignored. `interFE()` follows the same rules ([gsynth #13](https://github.com/xuyiqing/gsynth/issues/13), [gsynth #23](https://github.com/xuyiqing/gsynth/issues/23)).
* Covariates given both in the formula and in `X` stop, as do repeated names in `X` and `X` naming the outcome or the treatment.
* Non-numeric covariates stop (create numeric dummy columns first); logical covariates are used as 0/1.
* A character time index must hold numbers.
