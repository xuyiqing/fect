...
* Added `factors.from` parameter: `"notyettreated"` (default) or `"nevertreated"` controls which units provide the factor structure. Replaces `method = "gsynth"` with `method = "ife", factors.from = "nevertreated"`.
* CFE estimator now supports `factors.from = "nevertreated"`, enabling full CFE model components (additional FEs, $Z/\gamma$, $Q/\kappa$, latent factors) with never-treated estimation.
