...
**New ci.methods + per-type defaults.** `estimand()`'s `ci.method`
enum gains three values: `"bc"` (bias-corrected percentile, Efron
1987 minus the acceleration), `"bca"` (bias-corrected accelerated,
Efron 1987 in full --- adds an acceleration parameter computed via a
cell-level jackknife), and `"normal"` (Wald: `θ̂ ± z·SE`, what
`fit$est.att` already uses internally). The default is now `NULL`,
which triggers per-type defaults: `att` → `normal`,
`att.cumu` → `percentile`, `aptt` → `bca`, `log.att` → `bca`.
Existing scripts that pass `ci.method = ...` explicitly are
unaffected; callers that omit it will see different CI bounds for
`att.cumu` / `aptt` / `log.att` (point estimate and SE unchanged).

**New `para.error` argument for parametric bootstrap.** `fect()` gains
a `para.error = c("auto", "ar", "empirical", "wild")` argument that
selects the residual-error model used to draw bootstrap replicates
inside `vartype = "parametric"`. `"auto"` (default) resolves at fit
time: `"empirical"` on a fully-observed panel, `"ar"` on a panel with
missing cells; the resolved label is stored on `fit$para.error`.
`"ar"` is the v2.4.1 behavior (estimate an AR(1) process and draw
correlated residuals; available on any panel shape). `"empirical"`
resamples residuals i.i.d. from the main-fit residual pool.
`"wild"` applies Liu (1988) / Mammen (1993) / Cameron-Gelbach-Miller
(2008) unit-level Rademacher sign-flips over the empirical residual
pool; it preserves the marginal residual distribution and within-unit
dependence while sharing the parametric Loop-1 / Loop-2 architecture
(the bootstrap distribution is centered at the point estimate via the
same location-shift used by `"ar"` and `"empirical"`). `"empirical"`
and `"wild"` require a fully-observed panel; `"ar"` is the universal
fallback.

**Hard-error on bootstrap cell-drop pathology.** When a cell used in
the point estimate has `Y0_b ≤ 0` in `> 5%` of bootstrap replicates
for `log.att` (or any replicate where `E(Y0_b) ≈ 0` for `aptt`),
v2.4.2 now hard-errors with actionable guidance instead of silently
dropping cells in the per-replicate average. `estimand("log.att", ...)`
additionally hard-errors at the point-estimate level when any
treated cell has `Y_obs ≤ 0` or `Y0_hat ≤ 0` --- caller must
pre-transform (e.g. `log(Y + c)`) and refit before calling
`log.att`.

**Default `nboots` raised from 200 to 1000.** Affects `fect()`,
`fect.formula()`, `fect.default()`. Improves percentile-based
inference at the tails. Existing scripts that explicitly pass
`nboots = ...` are unaffected.
