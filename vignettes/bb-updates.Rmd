...
enum gains two values: `"bc"` (bias-corrected percentile, Efron 1987
minus the acceleration correction) and `"normal"` (Wald: `θ̂ ± z·SE`,
what `fit$est.att` already uses internally). The default is now
`NULL`, which triggers per-type defaults: `att` → `normal`,
`att.cumu` → `percentile`, `aptt` → `bc`, `log.att` → `bc`. Existing
scripts that pass `ci.method = ...` explicitly are unaffected;
callers that omit it will see different CI bounds for
...
dropping cells in the per-replicate average. The silent-drop
behavior contaminated the bootstrap distribution and produced
...
**Internal: warm-start infrastructure.** C++ solvers
(`inter_fe_ub`, `inter_fe_mc`) and the inner EM helpers gain an
optional `fit_init` parameter for warm-starting from a prior
prediction surface. Activation in `boot.R` was tested empirically
(cold vs warm comparison) and produced materially different
bootstrap distributions due to the non-convex EM converging to
different stationary points; activation deferred to v2.5.0 with a
properly designed approach. The infrastructure is in place but
unused by default; no behavioral change for users.

**Bug fix.** `R/diagtest.R`'s "F-test Failed. The estimated
covariance matrix is singular." message reworded to "F-test could
not be computed: the estimated covariance matrix is singular." The
previous wording conflated failure-to-reject with numerical failure.
