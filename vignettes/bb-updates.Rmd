...
* A factor time index is used in its level order, not sorted as text.
...
**Input checks.** Formula terms must be bare column names (`log(Y)`, `factor(g)`, `X1:X2` now stop); `X` cannot be combined with a formula; covariates must be numeric; a character time index must hold numbers; `cl` must be one column, constant within units, with at least two clusters. Each stop says what to do instead.
