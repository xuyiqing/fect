...
* **Default `r`**: `r` now defaults to `NULL`. For `method = "ife"` or `"gsynth"`, leaving out `r` now selects the number of factors by cross-validation over 0 to 5, instead of fitting zero factors.
* **Cross-validation**: each of these can change the selected `r` or `lambda` for some existing calls:
  * Calls that name variables as strings, such as `fect(Y = "y", D = "d", ...)`, now use the same defaults as formula calls (rolling cross-validation, `nlambda = 10`). Set `cv.method = "block"` to get the old folds.
  * `seed` now fixes the folds when `se = FALSE`. Before, it had no effect there.
  * With `se = TRUE`, cross-validation now uses `cv.rule`, `cv.buffer`, `cv.donut`, `min.T0` and `proportion` as given. `method = "gsynth"` now respects `cv.rule` ([#146](https://github.com/xuyiqing/fect/issues/146)).
  * `method = "cfe"` with `time.component.from = "nevertreated"` now respects `cv.rule`. With the default `"1se"` it can select a smaller `r`; `cv.rule = "1pct"` usually gives the old selection.
  * `criterion = "pc"` picks the lowest PC for never-treated fits ([gsynth #24](https://github.com/xuyiqing/gsynth/issues/24)).
  * With `CV = TRUE`, `W.agg` alone no longer weights never-treated fits ([gsynth #101](https://github.com/xuyiqing/gsynth/issues/101)).
  * With `parallel = FALSE`, `se = TRUE` draws the same folds as `se = FALSE`.
* **`estimand()` standard errors**: each bootstrap replicate is now read from its own units. The overall ATT, APTT, log-ATT and placebo or carryover standard errors were wrong unless the treated units came first in the data with one adoption time (3 to 6 times too small on `turnout`). `imputed_outcomes(replicates = TRUE)` had the same error ([#150](https://github.com/xuyiqing/fect/issues/150), [gsynth #37](https://github.com/xuyiqing/gsynth/issues/37), [gsynth #60](https://github.com/xuyiqing/gsynth/issues/60)).
* **Cumulative ATT**: `att.cumu()`, `effect()` and `estimand("att.cumu")` now return the running sum of the per-period ATTs, as documented. Before, at event time k they returned k times the average effect over all treated cells in event times 1 to k, which differs when the number of treated units changes. `att.cumu(weighted = TRUE)` gives the old number ([gsynth #75](https://github.com/xuyiqing/gsynth/issues/75)).
* **Parametric intervals**: `effect()`, `att.cumu()` and `estimand("att.cumu")` use the variance type stored on the fit. Their intervals were centered near zero when `vartype` was not typed as a string, and in `att.cumu()` always ([gsynth #37](https://github.com/xuyiqing/gsynth/issues/37), [gsynth #60](https://github.com/xuyiqing/gsynth/issues/60), [gsynth #75](https://github.com/xuyiqing/gsynth/issues/75)).
* **Parametric standard errors with `normalize = TRUE`** are no longer multiplied by the standard deviation of the outcome ([gsynth #14](https://github.com/xuyiqing/gsynth/issues/14)).
* **Never-treated fits with `se = TRUE`**: `method = "ife"`, `"fe"` and `"cfe"` with `time.component.from = "nevertreated"` use the never-treated estimator for the point estimate and every replicate, as `se = FALSE` does. Before, parts of the fit came from the not-yet-treated model.
* **Implied weights**: `wgt.implied` follows @Xu2017, so the weighted control outcomes rebuild the factor part of each treated unit's counterfactual. It has one row per control unit and one column per treated unit, also under `loading.bound = "simplex"`. Its values change ([gsynth #17](https://github.com/xuyiqing/gsynth/issues/17)).
* **Resampling a single unit**: with one treated unit, one reversal unit, or two never-treated units under the parametric bootstrap, the bootstrap now draws the right units. With one treated unit, most draws used to be dropped without a message.
* **Time index and covariates**: a factor time index is read in its level order, and a character index that holds numbers as those numbers ([gsynth #13](https://github.com/xuyiqing/gsynth/issues/13)). An exactly collinear covariate is dropped with a warning, with coefficient NA, instead of changing the estimate ([gsynth #80](https://github.com/xuyiqing/gsynth/issues/80), [gsynth #83](https://github.com/xuyiqing/gsynth/issues/83)).
* **Leave-one-out equivalence test**: the TOST test for `loo = TRUE` now uses the same pre-treatment window as the F test.

**Input checks that now stop or warn**

* Formula terms must be column names: `log(Y)`, `factor(g)`, `X1 * X2` and `.` stop with a message to create the column first. Intercept terms are ignored. `interFE()` follows the same rules ([gsynth #13](https://github.com/xuyiqing/gsynth/issues/13), [gsynth #33](https://github.com/xuyiqing/gsynth/issues/33)).
* Covariates given both in the formula and in `X` stop, as do repeated names in `X` and `X` naming the outcome or the treatment ([gsynth #23](https://github.com/xuyiqing/gsynth/issues/23)).
* Non-numeric covariates stop (create numeric dummy columns first); logical covariates are used as 0/1 ([gsynth #33](https://github.com/xuyiqing/gsynth/issues/33)).
* A character time index must hold numbers ([gsynth #13](https://github.com/xuyiqing/gsynth/issues/13)).
* Covariates that cannot be estimated (collinear, absorbed by the fixed effects, or with no variation on the fitted cells) are dropped with one warning, and their coefficients are NA. Absorbed covariates no longer stop `fect()`; `interFE()` still stops on them, now naming the right fixed effect ([gsynth #40](https://github.com/xuyiqing/gsynth/issues/40)).

**Bug fixes**

* Errors fixed: weights with the parametric bootstrap ([#150](https://github.com/xuyiqing/fect/issues/150), [gsynth #101](https://github.com/xuyiqing/gsynth/issues/101)); the cluster bootstrap with unequal clusters and `keep.sims = TRUE` ([gsynth #41](https://github.com/xuyiqing/gsynth/issues/41), [gsynth #86](https://github.com/xuyiqing/gsynth/issues/86)); bootstrap runs with few never-treated units ([gsynth #32](https://github.com/xuyiqing/gsynth/issues/32)); a cross-validation range larger than the never-treated units allow ([gsynth #32](https://github.com/xuyiqing/gsynth/issues/32)); `effect()` with one unit ([gsynth #45](https://github.com/xuyiqing/gsynth/issues/45), [gsynth #53](https://github.com/xuyiqing/gsynth/issues/53)); `att.cumu()` without standard errors; `method = "cfe"` with never-treated fitting and one treated unit, or with `W.agg` alone; counterfactual plots without standard errors; the factors plot with a Date time index.
* Clearer checks and messages: `cl` must be one column, constant within units, with at least two clusters, and parametric and jackknife fits ignore it with a warning; dropped bootstrap replicates are counted; a single candidate `r` skips cross-validation with the right message; fect stops when dropping periods without controls leaves no treated observation ([gsynth #57](https://github.com/xuyiqing/gsynth/issues/57)); `dloo` with never-treated fitting stops; the parametric bootstrap with fewer than two never-treated units stops.
* `loading.bound = "simplex"` is kept with cross-validation, with `method = "gsynth"` without standard errors, in the parametric bootstrap and in leave-one-out refits (which also reuse the fit's `gamma.loading`).
* `remove.id` lists the removed units whenever units are removed. `imputed_outcomes()` reports the aggregation weights of weighted fits (stored in `fit$W.agg`). A `cells` formula can use variables of the calling function.

**New features and other changes**

* New `dloo` and `dloo.adjust` flags on `fect()`, contributed by Anton Strezhnev: double leave-one-out pre-trend placebos, computed without re-fitting. With covariates, the covariate coefficients are re-estimated on each cohort's pre-treatment comparison sample. They require staggered adoption, a balanced pre-treatment panel, and `method = "fe"`. `dloo.adjust = TRUE` applies Ziyi Liu's correction.
* `cv.donut` is now checked against `cv.nobs` for block cross-validation. A value that left no held-out period to score now stops with a clear message instead of an internal error.
* User manual: the IFE leave-one-out example now sets `r` to 2 (thanks to Siyang Zhu, UPF, for the report). The Inference chapter's migration example now passes `data = data`.
...
* Tighter EM convergence defaults: `tol` from `1e-3` to `1e-5`,
  ...
  shift a few percent (up to 40% on factor-heavy CFE) closer to the
  EM's converged minimum. Pass `tol = 1e-3, max.iteration = 1000`
  explicitly to reproduce pre-v2.4.2 numerical output.
* `estimand()` warns when a tail-quantile CI method (`"basic"`,
  `"percentile"`, `"bc"`, `"bca"`) is requested on a fit with
  fewer than 1000 bootstrap replicates, recommending refit at
  `nboots = 1000`.
* `vartype = "jackknife"` with `Nco > 1000` emits a fit-time
  warning recommending `vartype = "bootstrap"` for tractability.
* Various bug fixes.
...
`esplot(..., Count = "n_cells")` draws the bottom-bar treated-cell
panel for cumulative plots.

## v2.4.0

(2026-04-29)

**New: post-hoc estimands API.** Two new public functions and one new
dedicated chapter in the user manual.

* `estimand(fit, type, by, ...)` typed dispatcher returns a tidy data
  frame with consistent columns regardless of estimand type. Shipped
  types:
  * `"att"` — default per-event-time ATT; byte-identical to
    `fit$est.att`.
  * `"att.cumu"` — cumulative ATT; replaces `effect()`.
  * `"aptt"` — average proportional treatment effect on the treated
    (Chen and Roth 2024 QJE).
  * `"log.att"` — mean log-scale treatment effect.
* `imputed_outcomes(fit, ...)` low-level accessor returns the cell-level
  potential-outcome surface as a long-form data frame for custom
  estimands. Pipe to dplyr / data.table for arbitrary aggregation.
* `cells = ` filter (logical / formula) on both functions, with
  `window = c(L, R)` sugar on `estimand()` for the common
...
to work byte-identically; removal not before v3.0.0. Numerical
equality with `estimand()` is asserted by package tests.

**Internal slot reservation.** `eff_debias` slot on the fit object is
now reserved (NULL for plain imputation estimators); populated by
future doubly-robust estimators so DR scores can be added without
breaking the long-form schema.
