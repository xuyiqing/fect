...
* New `dloo` and `dloo.adjust` flags on `fect()`, contributed by Anton Strezhnev: double, cohort-wise leave-one-out pre-trend placebos computed as a closed-form overlay on the in-sample fit, with no re-fitting. They fill the same slots as `loo` and plot with `plot(fit, dloo = TRUE)`. Requires staggered adoption, a balanced pre-treatment panel, and `method = "fe"`. `dloo.adjust = TRUE` applies Ziyi Liu's `(g-2)/(g-1)` correction.
* `method = "ife"` (or `"gsynth"`) called without `r` and without `CV = TRUE`
  ...
  `lambda`. The default `r` of 0 is unchanged.
...
* User manual, Factor-Based Methods chapter: the LOO pre-trend example now
  fits the IFE model with `r` set to 2. The previous call omitted `r` and ran with
