...
In this chapter, we explore various visualization options available in the **fect** package using data from @GS2020.
...
-   **Lines and Bounds:**
    -   `color`: The master color for estimate lines, points, and CI bands. In gap/exit/equiv plots, `color` sets the **post-treatment** color by default (pre-treatment defaults to gray). To control pre- and post-treatment colors independently, use `pre.color` and `post.color` instead.\
    -   `pre.color` / `post.color`: Explicitly set the colors for pre- and post-treatment estimates. These override `color` when specified. Default color conventions by plot type:
        -   **Gap plot**: pre-treatment = gray (in-sample), post-treatment = black (out-of-sample).
        -   **Exit plot**: pre-exit = black (out-of-sample), post-exit = gray (in-sample).
        -   **Gap/equiv plot with `loo = TRUE`**: all points = black (all out-of-sample).\
    -   `est.lwidth` / `est.pointsize`: Width and size of estimate lines and points.\
...
We can change the color of the post-treatment estimates (and their confidence intervals) using the `post.color` option. To change the pre-treatment color, use `pre.color`. The `color` option sets both the post-treatment color and CI bands simultaneously.

```{r preset-vibrant2}
plot(out.hh,
     preset = "vibrant", # Use vibrant colors
     post.color = "green4", # Color of the post-treatment estimates and CIs
...
The count histogram on the bottom of the graph shows the number of treated units in each period. To customize its color, outline color, and opacity, we can use the `count.color`, `count.outline.color`, and `count.alpha` options, respectively.
...
## esplot(): Standalone Event Study Plots

The `esplot()` function creates event study plots directly from a data frame, without requiring a fitted `fect` object. This is useful when you have pre-computed ATT estimates (e.g., from another package or from manual calculations) and want to produce publication-quality event study figures using the same styling as **fect** plots.

### Basic usage

`esplot()` expects a data frame with columns for the time period, the point estimate, and optionally confidence interval bounds and observation counts. By default, it looks for columns named `"ATT"`, `"CI.lower"`, and `"CI.upper"`.

```{r esplot_basic, fig.width = 6, fig.height = 4.5}
...
Setting `connected = TRUE` displays the estimates as a connected line with a confidence band:

```{r esplot_connected, fig.width = 6, fig.height = 4.5}
esplot(es_data, Period = "Time",
       connected = TRUE,
...
The `highlight.periods` argument highlights specific time periods in the plot, which is useful for drawing attention to placebo or treatment periods:

```{r esplot_highlight, fig.width = 6, fig.height = 4.5}
esplot(es_data, Period = "Time",
