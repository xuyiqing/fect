...
The **loading.overlap** plot is a companion diagnostic to the bounded-loadings feature (see [Chapter @sec-gsynth] for the underlying simplex projection). It answers: do treated units lie inside the support of the controls in factor space?
...
For $r = 1$, the same diagnostic is rendered as a mirror histogram: treated counts above the axis, control counts flipped below, with a vertical band marking the range of control loadings. The 1D analog of "outside the hull" is "outside the band." The plot is informative even when `loading.bound = "none"` --- it shows whether the constraint *would* bind if activated.
