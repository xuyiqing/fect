...
The standard FEct estimator (`method = "fe"`) assumes that the untreated potential outcome is driven by unit and time fixed effects plus observed time-varying covariates:

$$Y_{it}^{0} = \alpha_i + \xi_t + X_{it}'\beta + e_{it}$$
...
4. Latent factors drive outcomes ($\lambda' f$).

The CFE estimator models the untreated potential outcome as:

$$Y_{it}^{0} = \mu + X_{it}'\beta + Z_i'\gamma_{g(t)} + \kappa_i' Q_t + \alpha_i + \xi_t + \omega_{k(i)} + \sum_{m=1}^{r} \lambda_{im} f_{tm} + e_{it}$$
...
```{r cfe-42-dgp, eval = TRUE}
set.seed(42)
N <- 500; TT <- 20; n.groups <- 5

# Assign regions (equal-sized groups)
...
# Region-specific linear time trends (confounding source)
region_slope <- c(-1, -0.5, 0, 0.5, 1) * 0.08
delta_gt <- outer(region_slope, (1:TT) - TT / 2)

# Treatment: probability and timing depend on region
...
dat_fe$region_time <- interaction(dat_fe$region, dat_fe$time, drop = TRUE)
...
In the `index` argument, elements beyond the first two (unit, time) are treated as additional fixed-effect grouping variables. You can include multiple additional groupings: `index = c("id", "time", "region", "sector")`.
:::
...
simdata_lin <- expand.grid(time = 1:TT_lin, id = 1:N_lin)
simdata_lin$Y <- alpha_lin_i[simdata_lin$id] + xi_lin_t[simdata_lin$time] +
                 kappa_lin_i[simdata_lin$id] * Q_lin_t[simdata_lin$time] +
                 rnorm(N_lin * TT_lin, 0, 1)
simdata_lin$D <- as.integer(is_treated_lin[simdata_lin$id] == 1 &
                            simdata_lin$time >= T0_lin)
simdata_lin$Y <- simdata_lin$Y + tau_lin * simdata_lin$D
```
...
out.fe.lin <- fect(Y ~ D, data = simdata_lin,
  index = c("id", "time"),
  method = "fe", force = "two-way",
  se = TRUE, parallel = TRUE, nboots = 200,
  placeboTest = TRUE, placebo.period = c(-2, 0))
...
out.cfe.lin <- fect(Y ~ D, data = simdata_lin,
  index = c("id", "time"),
  method = "cfe", force = "two-way",
  Q.type = "linear",
  se = TRUE, parallel = TRUE, nboots = 200,
...
# True time trend: half-cycle sin wave (lower frequency)
