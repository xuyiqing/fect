...
**`method = "gsynth"` is not a separate estimator.** It is a convenience wrapper that sets defaults appropriate for the original Generalized Synthetic Control method: `factors.from = "nevertreated"`, `em = FALSE`, and auto-selects the number of factors via cross-validation. Internally, it runs the IFE estimator.
...
| Want to reproduce the original gsynth | `method = "gsynth"` (or equivalently: `method = "ife"`, `factors.from = "nevertreated"`, `em = FALSE`) |
