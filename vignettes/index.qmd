...
### Estimation Strategy: `factors.from`

In **fect**, the `factors.from` parameter controls which research setting governs estimation:

-   `factors.from = "notyettreated"` (default) — the DID setting. Uses all not-yet-treated observations to estimate factors via EM.
-   `factors.from = "nevertreated"` — the synth setting. Uses only never-treated controls.
-   `method = "gsynth"` — a convenience wrapper that sets `factors.from = "nevertreated"` and auto-selects the number of factors. It is equivalent to `method = "ife"` with `factors.from = "nevertreated"`.

### Estimator Families

The **fect** package provides two families of counterfactual estimators, each compatible with both research settings:

-   **The IFE family** (Interactive Fixed Effects): `method = "ife"` ($r \geq 0$ latent factors; $r = 0$ reduces to TWFE), `method = "fe"` (alias for IFE with $r = 0$), and `method = "cfe"` (extends IFE with additional known structure — grouped time coefficients, unit-specific trends, extra fixed effects).
-   **Matrix completion**: `method = "mc"` — imputes counterfactuals via nuclear norm minimization without explicit factor structure. Only compatible with the DID setting (`factors.from = "notyettreated"`).
...
-   [Chapter @sec-fect]\
    This chapter explains how to apply the main estimators and diagnostic tests in **fect**. It introduces the two estimation settings (`factors.from`) and separates estimation from inference options.

-   [Chapter @sec-cfe]\
    This chapter provides a dedicated tutorial for the Complex Fixed Effects (CFE) estimator, which extends IFE with multi-level fixed effects, time-invariant covariates with time-varying coefficients, unit-specific time trends, and interactive fixed effects.

-   [Chapter @sec-plots]\
    In this chapter, we explore various plotting options available in **fect**.

-   [Chapter @sec-gsynth]\
    This chapter demonstrates the Gsynth program — the synthetic control setting using `factors.from = "nevertreated"` — originally developed in the **gsynth** package.
