...
This chapter demonstrate the generalized synthetic control method, or Gsynth, proposed in @Xu2017 (\[<a href="https://www.cambridge.org/core/journals/political-analysis/article/generalized-synthetic-control-method-causal-inference-with-interactive-fixed-effects-models/B63A8BD7C239DD4141C67DA10CD0E4F3" target="_blank">Paper</a>\]).
...
### Key Equivalence
`fect(..., method = "gsynth")` is equivalent to `fect(..., method = "ife", factors.from = "nevertreated")`.

Gsynth is not a separate estimator — it is the IFE model estimated using only never-treated controls. The **gsynth** package was developed in the synthetic control research setting, where a small number of treated units are compared against never-treated controls to learn latent time effects.
:::

### When to use the never-treated regime

Use `factors.from = "nevertreated"` (or equivalently `method = "gsynth"`) when:
...
-   You do **not** want pre-treatment data from treated units to influence factor estimation — this improves computational speed and stability.

In contrast, the default regime (`factors.from = "notyettreated"`, used in [Chapter @sec-fect]) accommodates treatment reversal and uses EM to estimate factors from all not-yet-treated observations.
...
```{r sim-panelview-status, cache = FALSE, fig.height=7, fig.width=7}
library(panelView)
...
## Cross-validation in the nevertreated regime
...
Under `factors.from = "nevertreated"`, the CFE estimator uses only never-treated controls for factor estimation — the same principle as gsynth. The additional CFE components (extra FE, time trends, Z/gamma) are estimated on the control panel and projected onto treated units alongside the interactive factors. See [Chapter @sec-cfe] for the full CFE tutorial.
:::

### Comparing gsynth and CFE + nevertreated

Both approaches share the same estimation regime (never-treated controls only), but CFE adds structural components. Whether the extra flexibility helps depends on the data generating process. If the true model has unit-specific trends or group fixed effects, CFE with nevertreated will outperform plain gsynth. If the true model is pure IFE, the additional CFE parameters add noise without benefit. The `fect_mspe()` function provides a principled comparison:

```{r cfe_nt_vs_gsynth, eval=TRUE, cache=TRUE, message=FALSE, results='hide'}
out.gsynth.comp <- fect(Y ~ D + X1 + X2, data = simgsynth, index = c("id","time"),
                        method = "gsynth", force = "two-way",
                        r = 2, se = FALSE, CV = FALSE)

out.cfe.nt.comp <- fect(Y ~ D + X1 + X2, data = simgsynth, index = c("id","time"),
                        method = "cfe", force = "two-way",
                        factors.from = "nevertreated",
                        r = 2, se = FALSE, CV = FALSE)
```
...
mspe.comp <- fect_mspe(list(out.gsynth.comp, out.cfe.nt.comp), seed = 1234)
print(mspe.comp$summary)
```

Since `simgsynth` follows a pure IFE data generating process, gsynth and CFE with nevertreated should produce similar MSPE, confirming that the additional CFE structure neither helps nor hurts when it is not needed.
