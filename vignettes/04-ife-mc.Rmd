...
By default (`cv.rule = "1se"`), **fect** picks the smallest `r` whose cross-validation error is within one standard error of the lowest error, with the standard error taken across the `k` folds. This keeps factors out when they improve the fit by less than the fold-to-fold noise. `cv.rule = "min"` picks the `r` with the lowest error, and `cv.rule = "1pct"` picks the smallest `r` within 1% of the lowest error, the rule used before v2.3.0.
...
We set `max.iteration = 20000` to allow full convergence of the IFE model with `tol = 1e-5`. 
...
The IFE call sets `max.iteration = 20000`. Placebo and carryover tests drop a window of cells from the IFE estimation, which slows EM convergence below the default cap of 5000 iterations. With the smaller effective sample, EM here converges around iteration 8500. The same argument is used in the carryover and `carryover.rm` calls below. The MC fits do not need it, since matrix completion does not run the same EM loop.
...
As with the placebo test, the LOO test is run on a settled model, so we fix the tuning parameters at the values selected earlier (`r` set to 2 for IFEct, `lambda` set to `out.mc$lambda.cv` for MC) and turn cross-validation off. Since v2.4.6, `method = "ife"` without `r` selects the number of factors by cross-validation (0 to 5); with `CV = FALSE` and no `r`, it falls back to zero factors, the FEct model, with a message.

With covariates, each leave-one-out fit still estimates the covariate coefficients from all remaining untreated observations, as the imputation estimator always does. These include the control units' observations after the treated units adopt, so a LOO estimate is out of sample for the held-out period but can still move with later data through those coefficients. Under staggered adoption with `method = "fe"`, the double leave-one-out placebos of [Chapter @sec-fect] re-estimate the coefficients on pre-treatment data only.
...
In the IFEct panel, the $F$ test does not reject the null of no pre-trend (p = 0.82), and the TOST rejects the null that some pre-treatment average lies outside the equivalence range (p < 0.001). In the MC panel, the $F$ test rejects (p < 0.001): the LOO estimates are small but drift upward in the last few periods before the treatment starts. The TOST also rejects non-equivalence (p < 0.001), so all of the MC pre-treatment averages stay inside the equivalence range. IFEct is the more suitable model for these data.
...
out.ife.rm.test <- fect(Y ~ D + X1 + X2, data = simdata, index = c("id", "time"),
  ...
  nboots = 200, carryoverTest = TRUE, carryover.period = c(1, 3),
  max.iteration = 20000)# remove three periods
```
...
| Double LOO pre-trend test | Cohort-wise leave-one-out placebos from the in-sample fit, no re-fitting; `method = "fe"` with staggered adoption only (see [Chapter @sec-fect]) | `dloo = TRUE` (in `fect()`), `dloo = TRUE` (in `plot()`) | `"gap"` or `"equiv"` | `F.p`, `equiv.p` |
...
- A **larger** F-test, placebo, or carryover p-value means the test does not reject the null of no pre-trend (or no carryover effect). Not rejecting is weaker than showing that there is none; the equivalence tests address that.
...
- Under staggered adoption with `method = "fe"`, `dloo = TRUE` gives the corrected placebos at no re-fitting cost; see [Chapter @sec-fect].
...
* **Weight only in the aggregation.** Pass `W.agg = "your_column"` alone. The outcome model is fit unweighted (also when `CV = TRUE`) and the weight is applied only when summarizing across treated cells. Common cases include calibration weights to a target population, or post-stratification weights that should adjust the summary but not the model fit.
