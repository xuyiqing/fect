...
The examples below use a small synthetic panel with positive outcomes so that log-scale calculations are well defined. The fit sets `keep.sims = TRUE` so that the cell-level imputed counterfactuals from each bootstrap replicate are saved on the fit object. APTT and log-scale ATT need these saved values to recompute their non-linear formulas inside each replicate. The overall ATT and the cumulative ATT by event time need them as well. Only the per-event-time ATT and the cumulative ATT over one window work from the per-period bootstrap results that fect always keeps (see the section on saving bootstrap runs below).
...
The cumulative ATT through each event time. Each row reports the running sum of the per-period ATTs up to that event time, where each per-period ATT is the average effect over the treated cells at that event time. This call replaces the legacy `effect()` function and produces identical numbers.

Before v2.4.6, fect reported a count-weighted version instead: k times the average effect over all treated cells in event times 1 to k. The two agree when every event time has the same number of treated cells. Here they agree through event time 8, which every treated unit reaches, and differ after it. `att.cumu(fit, weighted = TRUE)` still returns the count-weighted number.
...
Each row is one treated cell. The columns are the unit and time coordinates, the observed outcome, the imputed counterfactual, the cell-level effect (the cell's contribution to the ATT estimator), and the aggregation weight. Setting `replicates = TRUE` returns the treated cells of every bootstrap replicate instead. A replicate's rows are the treated cells of the units it drew: a unit drawn twice appears twice, and a unit not drawn does not appear. So the number of rows varies across replicates, and the mean of `eff` within a replicate is that replicate's overall ATT.
...
all.equal(as.numeric(tapply(po_rep$eff, po_rep$replicate, mean)),
...
Two calls work without it, because they read the per-period bootstrap results that fect always keeps: the per-event-time ATT, `estimand(fit, "att", "event.time")`, and the cumulative ATT over one window, `estimand(fit, "att.cumu", "overall", window = ...)`. The other calls recompute their summaries from the cell-level array and need the saved runs: the overall ATT, the cumulative ATT by event time, APTT, log-scale ATT, their placebo and carryover versions, and `imputed_outcomes(replicates = TRUE)`.
...
For the other types, `estimand()` builds each replicate's value
from `fit$eff.boot`. Parametric replicates are simulated with no
treatment effect, so their values center near zero, not at the point
estimate. For `"att"` with `by = "overall"` or with a placebo or
carryover test, and for `"aptt"` and `"log.att"`, `estimand()`
shifts the replicate values to center on the point estimate before
it forms the interval. For `"att.cumu"`, it returns the intervals of
`effect()` (by event time) and `att.cumu()` (one window); for
parametric fits these are symmetric intervals around the estimate,
built from the bootstrap standard error.
