...
plus or minus the normal critical value times the SE.
...
For the cumulative ATT the replacements give the same numbers to all decimals: `estimand(fit, "att.cumu", "event.time")` returns what `effect(fit, cumu = TRUE)` returns, and the overall form returns the last row of `att.cumu()`. For `effect(fit, cumu = FALSE)`, the point estimates and standard errors equal those of `estimand(fit, "att", "event.time")`. For bootstrap fits the intervals differ: `effect()` takes the 2.5% and 97.5% quantiles of the bootstrap replicates, while `estimand()` returns the normal intervals of `fit$est.att`. In the example above they differ by up to 0.57; at event time 1 they are [4.01, 5.26] and [3.99, 5.23]. For parametric and jackknife fits both give the normal intervals (with the fit's default `ci.method = "normal"`), so they agree. The other difference is that `estimand()` returns a data frame instead of the legacy matrix.
