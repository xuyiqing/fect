...
## Placebo and carryover tests

Issue #131 asked for placebo and carryover versions of every alternative estimand, not just the default `att.placebo` / `att.carryover` scalars. Starting in v2.4.2 the dispatcher takes `test = c("none", "placebo", "carryover")`. The placebo path requires a fit produced with `placeboTest = TRUE` (and a placebo period); the carryover path requires `carryoverTest = TRUE` plus a panel with treatment reversals.

```{r setup-placebo-carryover, eval = TRUE, message = FALSE, results = 'hide'}
...
The carryover series uses cells in `fit$carryover.period` --- the early post-reversal window where the assumption that the treatment effect dissipates immediately would imply zero estimates. Non-zero carryover estimates suggest the effect persists past treatment removal.
