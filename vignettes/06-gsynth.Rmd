...
`out$wgt.implied` ($N_{co}\times N_{tr}$) stores the implied weights of all control units for each treated unit: one row per control unit and one column per treated unit, named by unit. They follow @Xu2017, so the weighted control outcomes rebuild the factor part of each treated unit's counterfactual. Different from the synthetic control method, the weights can take both positive and negative values. Below we show the control unit weights for Wisconsin.
...
Each treated counterfactual $\hat Y_i(0) = F\,\hat\lambda_i$ is a linear combination of control-implied outcomes. The coefficients on the controls are the *implicit donor weights*, stored in `wgt.implied`: an $N_{co} \times N_{tr}$ matrix with one row per control unit and one column per treated unit, in both fits. Under `loading.bound = "none"`, these come from the Moore-Penrose pseudo-inverse and may be negative, exceed one, or vary wildly with small perturbations. Under `loading.bound = "simplex"`, each column is non-negative and sums to one (a proper synthetic-control weighting). The scatter below shows every (treated unit, control unit) pair: x-axis is the unbounded weight, y-axis is the bounded weight.
...
3. **Bounded weights concentrate on a few donors.** In this example every unbounded weight is small, between about $-0.11$ and $0.10$: the pseudo-inverse spreads each treated unit's weight over all 45 controls. The simplex weights lie between zero and one and give a few donors much larger weights, so those donors' points lie well above the dashed line.
...
`sim_gsynth` follows an IFE data generating process. In the unbounded fit, four of the five treated units have loadings inside the convex hull of the control loadings, and one lies outside it. The two fits give ATTs of 4.64 (unbounded) and 5.10 (bounded); the true ATT in `sim_gsynth` is 5.46. The gap between the two fits, 0.46, is about half of either fit's standard error (about 0.9). The *weight* representations differ much more than the estimates, which is informative on its own: the bounded weights are non-negative and concentrate on fewer donors. Real datasets with sparse donor pools (few treated units alongside controls with disparate factor loadings) typically show larger ATT gaps; in those settings, the simplex bound corrects the extrapolation that pure least-squares performs and provides a more honest synthetic-control counterfactual.
