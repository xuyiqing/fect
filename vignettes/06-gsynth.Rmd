...
This chapter demonstrates the generalized synthetic control method, or Gsynth, proposed in @Xu2017 \[<a href="https://www.cambridge.org/core/journals/political-analysis/article/generalized-synthetic-control-method-causal-inference-with-interactive-fixed-effects-models/B63A8BD7C239DD4141C67DA10CD0E4F3" target="_blank">Paper</a>\]. R script used in this chapter can be downloaded [here](https://raw.githubusercontent.com/xuyiqing/fect/dev/vignettes/rscript/06-gsynth.R).
...
### Recommended changes in v2.3.0 for Gsynth users
...
The legacy values `"all_units"` (now `"block"`) and `"treated_units"` (block masking restricted to treated pre-treatment cells) are still accepted but emit a deprecation message; both will be replaced by the unified `(cv.method, cv.units)` API in v2.4.0. See the [cheatsheet](aa-cheatsheet.html#sec-cheatsheet-cv) for the deprecation plan.

### Rolling-window CV 

Set `cv.method = "rolling"` in the main `fect()` CV dispatcher to use rolling-window CV (the recommended default for serially correlated panels --- see the recommended-changes callout at the top of this chapter for the empirical motivation, and [Chapter @sec-ife-mc] for the design and the side-by-side figure).

```{r rcv_dispatcher_gsc, eval = FALSE}
...
`out$wgt.implied` stores the implied weights of all control units for each treated unit: an $N_{co}\times N_{tr}$ matrix with one row per control unit and one column per treated unit, labeled by unit ID. Column $i$ holds the weights $w_i$ that rebuild the factor part of treated unit $i$'s counterfactual from the control units' factor parts, $\Lambda_{co}^\top w_i = \lambda_i$, with $w_i = \Lambda_{co}(\Lambda_{co}^\top\Lambda_{co})^{-1}\lambda_i$ [@Xu2017]. Different from the synthetic control method, the weights can take both positive and negative values; with time fixed effects in the model, each column sums to zero. Below we show the control unit weights for Wisconsin.

```{r turnout-implied-weights}
dim(out_turnout$wgt.implied)
sort(out_turnout$wgt.implied[, "WI"])
```
...
For staggered adoption without treatment reversals, the cumulative ATT summarizes how the total treatment dose accumulates over relative time. It is useful for outcomes where each additional treated period adds to a running total (e.g., cumulative voter mobilization). The cumulative ATT at event time $k$ is the running sum of the per-period ATTs from event time 1 to $k$. Because `out_turnout` was estimated with `keep.sims = TRUE` and `vartype = "parametric"`, the per-cell replicate surface needed for `estimand()` is available.

```{r turnout-cumulative, fig.height=5, fig.width=7}
...
See [Chapter @sec-estimands] (Alternative Estimands) for the full `estimand()` API including parametric variance, log-scale ATT, and APTT.
...
Each treated counterfactual $\hat Y_i(0) = F\,\hat\lambda_i$ is a linear combination of control-implied outcomes. The coefficients on the controls --- stored in `wgt.implied`, an $N_{co} \times N_{tr}$ matrix with one column per treated unit --- are the *implicit donor weights*. Under `loading.bound = "none"`, they come from the Moore-Penrose pseudo-inverse, $w_i = \Lambda_{co}(\Lambda_{co}^\top\Lambda_{co})^{-1}\lambda_i$, and can be negative; with time fixed effects in the model, each column sums to zero. Under `loading.bound = "simplex"`, each column is non-negative and sums to one (a proper synthetic-control weighting). Both matrices have the same layout, so the scatter below pairs the two weights of every (treated unit, control unit) pair: x-axis is the unbounded weight, y-axis is the bounded weight.
...
3. **Large bounded weights go to donors the pseudo-inverse already favors.** The points well above $y = 0$ all have positive unbounded weights, among the largest of their treated unit. For most treated units, the simplex puts almost all the weight on a few of these donors, and no bounded weight can exceed one. The unbounded weights, which sum to zero, stay small and are spread over the whole donor pool.
...
`sim_gsynth` follows an IFE data generating process. In the unbounded fit, four of the five treated units have loadings inside the convex hull of the control loadings, and one lies outside it. The two fits give ATTs of 4.64 (unbounded) and 5.10 (bounded); the true ATT in `sim_gsynth`, the mean of its `eff` column over the treated cells, is 5.46. The gap between the two fits, 0.46, is about half of either fit's standard error (0.8 to 1.0, depending on the bootstrap draws). The *weight* representations differ much more than the estimates, which is informative on its own: the bounded weights are non-negative and concentrate on fewer donors. Real datasets with sparse donor pools (few treated units alongside controls with disparate factor loadings) typically show larger ATT gaps; in those settings, the simplex bound corrects the extrapolation that pure least-squares performs and provides a more honest synthetic-control counterfactual.
