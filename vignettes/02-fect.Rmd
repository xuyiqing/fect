...
Using the current version of **fect**, we can apply several different methods to make counterfactual predictions and estimate treatment effects by specifying the `method` option: `"fe"` (two-way fixed effects, default), `"ife"` (interactive fixed effects), `"mc"` (matrix completion method), and `"cfe"` (complex fixed effects). First, we illustrate the main syntax of **fect** using the `"fe"` method.

The two-way fixed effects counterfactual estimator (FEct) is also independently proposed by @BJS2024 and @gardnertwo, who refer to it as the "imputation method" and "two-stage DID," respectively.

### Two estimation regimes {#fect-regimes}

The `factors.from` parameter determines which observations are used to estimate the factor model. See the [introduction](index.qmd#two-estimation-regimes) for full details. Here we briefly demonstrate both regimes on `simdata`.

The **default regime** (`factors.from = "notyettreated"`) uses all not-yet-treated observations via the EM algorithm. This is the regime used by all examples in this chapter:

```{r regime_nyt, eval=TRUE, cache = TRUE, message = FALSE, results = 'hide'}
out.nyt <- fect(Y ~ D + X1 + X2, data = simdata, index = c("id","time"),
                method = "ife", force = "two-way", CV = TRUE, r = c(0, 5))
```

The **never-treated regime** (`factors.from = "nevertreated"`) uses only never-treated controls. It is ideal for settings without treatment reversal:

```{r regime_nt, eval=TRUE, cache = TRUE, message = FALSE, results = 'hide'}
out.nt <- fect(Y ~ D + X1 + X2, data = simdata, index = c("id","time"),
               method = "ife", factors.from = "nevertreated",
               force = "two-way", CV = TRUE, r = c(0, 5))
```

::: {.callout-note appearance="simple"}
Since `simdata` contains treatment reversals, the never-treated regime may not be the best choice here — it is shown for illustration only. For a dataset without reversals, see [Chapter @sec-gsynth] which uses `simgsynth`.
:::
...
The key parameters that control the **model** are: `method` (estimator choice), `force` (fixed effects structure), `r` (number of factors), `factors.from` (estimation sample), and `em` (EM algorithm). These determine how $\hat{Y}(0)$ is imputed.
:::
...
::: {.callout-note appearance="simple"}
### The `em` parameter
By default, `em = TRUE` and the EM algorithm is used to estimate the factor model when the estimation sample has missing entries (which is always the case under `factors.from = "notyettreated"`). When `factors.from = "nevertreated"`, the estimation sample is complete by construction, so the `em` parameter has no effect.
:::
