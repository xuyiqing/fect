...
The key parameters that control **uncertainty estimation** are: `se` (enable standard errors), `vartype` (`"bootstrap"`, `"jackknife"`, `"parametric"`, or `"wild"`), `nboots` (number of bootstrap replications), and `cl` (clustering variable). These parameters do not affect point estimates.
:::

::: {.callout-note appearance="simple"}
**Variance methods at a glance.** Four `vartype` values are supported. They differ in *what* gets randomized per replicate, *how* the unit composition is preserved, and *what* the relative computational cost is (in multiples of one model refit on the full data). Each replicate refits the model end-to-end; `cost` below is the multiple of `nboots` refits incurred relative to `bootstrap`.
...
| `"parametric"` | Draw model errors from estimated covariance and re-impute | Pseudo-treated drawn from never-treated pool | ≈ 1x | Synth setting only; `time.component.from = "nevertreated"`, no reversal, not MC. See [Section @sec-parametric-regimes] |
| `"wild"` (new in v2.4.2) | Unit-level Rademacher signs ±1 on main-fit residuals (`D = 0` cells) | Preserved exactly --- no unit dropout | 1x | Continuous outcomes; `method ∈ {fe, ife, mc, gsynth, cfe}`; respects within-unit dependence |

The default `"bootstrap"` is robust and the right starting point. Switch to `"jackknife"` when `N` treated is small (single digits). Switch to `"wild"` when unit resampling would distort cohort balance --- e.g., very few treated units in some cohorts. Switch to `"parametric"` only inside the gsynth-style regime where the assumptions in [Section @sec-parametric-regimes] hold.
:::
