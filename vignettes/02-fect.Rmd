...
Under staggered adoption, @li2025benchmarking show that the in-sample placebo has two problems. First, it is attenuated: the tested period helps estimate the unit's own fixed effect, so the residual at that period is pulled toward zero. Second, it is contaminated: cohorts that adopted earlier serve as controls for a later cohort's pre-treatment periods while they are already treated. The leave-one-out approach above removes the first problem by re-fitting the model once per pre-treatment period. Their proposed fix, the double leave-one-out placebo, removes both. For a cohort adopting at period $g$ and a pre-treatment period $t$, it drops period $t$ and compares that cohort only with units that adopt strictly later or never, as a two-by-two difference in means. Because unit and time fixed effects cancel in that difference, the estimator is a fixed linear function of the in-sample placebos that **fect** already computes. It therefore costs nothing beyond the original fit. Anton Strezhnev contributed the implementation; set `dloo = TRUE` in `fect()` to use it.
