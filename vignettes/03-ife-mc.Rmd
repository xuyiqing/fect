...
Both `"all_units"` and `"treated_units"` use random anchors to mask contiguous blocks of `cv.nobs` observations. With residuals that are temporally correlated, this design has a subtle problem: the blocks are immediately adjacent to training observations on both sides, so a high-rank model can "predict" the held-out block by exploiting the AR structure that leaks across the train/test boundary. Even widening `cv.nobs` and adding a `cv.donut` may not fully close this channel.
