...
Both `"all_units"` and `"treated_units"` use random anchors to mask contiguous blocks of `cv.nobs` observations. With residuals that are temporally correlated, this design has a subtle problem: the blocks are sandwiched between training observations on both sides, so a high-rank model can "predict" the held-out block by exploiting the AR structure that leaks across the train/test boundary. Even widening `cv.nobs` and adding a `cv.donut` may not fully close this channel.

`r.cv.rolling()` implements the standard time-series rolling-window CV design (cf. `forecast::tsCV`, `tidymodels::sliding_window`, `caret::createTimeSlices`) adapted to panel data. For each of `k` folds and each eligible control unit, a random anchor time `t*` is sampled. The fold's training set excludes (i) the `cv.nobs` cells starting at `t*` (the held-out block, scored for MSPE), (ii) `cv.buffer` cells immediately before `t*` (past-side buffer, analogous to `cv.donut` but only on the past side), and (iii) all cells from `t* + cv.nobs` onward (rolling-window step --- training cannot see the future of the held-out block). MSPE is averaged across folds; the fold-SE feeds the `"1se"` rule.

```{r rcv_rolling_demo, eval=FALSE}
res <- r.cv.rolling(Y ~ D + X1 + X2, data = simdata,
                    index = c("id", "time"),
                    method = "ife", r.max = 5,
...
`r.cv.rolling()` supports `method = "ife"` (IFE-EM, internally `time.component.from = "notyettreated"`) and `method = "gsynth"` (GSC, internally `time.component.from = "nevertreated"`). Both paths populate `Y.ct.full` at masked control positions, so rolling CV can score MSPE on either estimator. Other methods (e.g., `"mc"`) are not yet supported.

::: {.callout-note appearance="simple"}

### When to prefer rolling CV

If your residuals are notably autocorrelated (e.g., post-fit residual AR(1) above ~0.4), the random-anchor strategies can over-select `r` because the contiguous-block masking leaks correlated information. `r.cv.rolling()` is more conservative in those settings. For approximately i.i.d. residuals, the random-anchor strategies and rolling CV typically agree on `r.cv`.
...
| `cv.donut` | 0 | Periods excluded around treatment onset in the evaluation set to minimize contamination |
| `min.T0` | 5 | Minimum pre-treatment observations required per unit |

The block removal (`cv.nobs`) addresses serial correlation: instead of masking individual observations, we mask blocks of consecutive observations. The `cv.donut` option further excludes observations near the treatment boundary from the test set to avoid contamination due to temporal correlation in data.
...
Cross-validation can be computationally expensive, especially with `cv.method = "all_units"` (which re-estimates the full factor or low-rank model `k` times per candidate tuning parameter). Parallel computing is enabled by default (`parallel = TRUE`); on large panels, the CV step engages multiple workers automatically.
...
The unit of parallel work is the (rank, fold) pair (or (lambda, fold) for MC), dispatched flat across workers via `future_lapply`. This gives near-linear speedup up to the number of total tasks: e.g., with `r=c(0,5)` and `k=10`, there are 60 tasks per CV invocation. The `"treated_units"` masking strategy and the `"loo"` method are always sequential, because their per-fold computations depend on rank-specific quantities that cannot be batched.
