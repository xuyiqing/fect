...
The package **fect** provides the option balance.period which allows the calculation of the average treatment effects only for units that exhibit complete data in specified pre- and post-treatment periods. For instance, if the option is set to `balance.period = c(-4,3)`, the program will calculate the average treatment effects for units that have at least four consecutive non-missing observations in the pre-treatment periods (-4, -3, -2, -1) and at least four consecutive non-missing observations in the post-treatment periods (0, 1, 2, 3).
