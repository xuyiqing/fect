...
p <- plot(out.fect, main = "Estimated ATT (FEct)", ylab = "Effect of D on Y", 
...
p <- plot(out.ife, main = "Estimated ATT (IFEct)")
```
...
```{r fig.width = 6, fig.height = 4.5}
p <- plot(out.mc, main = "Estimated ATT (MC)")
```  

### Heterogeneous treatment effects

One way to explore treatment effects heterogeneity is to use a series of box plots to visualize the estimated individualistic treatment effects of observations under the treatment condition (by setting `type = "box"`). Although these effects are not identified at the observation level, their distribution reveals important information of treatment effects heterogeneity.
```{r hte, eval = TRUE, cache = TRUE, warning = FALSE, fig.width = 6, fig.height = 4.5}
p <- plot(out.ife, type = "box", xlim = c(-15, 10))
```
...
p <- plot(out.fect.loo, type = "equiv", ylim = c(-4,4), loo = TRUE,
     cex.legend = 0.6, main = "Testing Pre-Trend LOO (FEct)", cex.text = 0.8)

p <- plot(out.ife.loo, type = "equiv", ylim = c(-4,4), loo = TRUE,
     cex.legend = 0.6, main = "Testing Pre-Trend LOO (IFEct)", cex.text = 0.8)

p <- plot(out.mc.loo, type = "equiv", ylim = c(-4,4), loo = TRUE, 
...
p <- plot(out.fect, type = "exit", ylim = c(-2.5,4.5), main = "What Happens after the Treatment Switches Off?")
```

```{r exit_2, eval = TRUE, cache = TRUE, warning = FALSE, fig.width = 6, fig.height = 4.5}
p <- plot(out.ife, type = "exit", ylim = c(-2.5,4.5), main = "Exit Plot (IFE)")
```

```{r exit_3, eval = TRUE, cache = TRUE, warning = FALSE, fig.width = 6, fig.height = 4.5}
p <- plot(out.mc, type = "exit", ylim = c(-2.5,4.5), main = "Exit Plot (MC)")
```
...
p <- plot(out.fect, type = 'status', axis.lab = "time", cex.axis  = 0.6)
```
...
p <- plot(out.fect.p, type = 'status', axis.lab = "both", id = c(101:120), cex.axis  = 0.6)
```
...
p <- plot(out.fect.c, type = 'status', axis.lab = "off", gridOff = TRUE)
```
...
p <- plot(out.ife.rm.test, type = 'status', axis.lab = "off", gridOff = TRUE)
```
...
p <- plot(out.ife.g, show.group = "Cohort:22", 
