...
- treated counterfactual plot (`counterfactual`)
- diagnostic plots (`equiv`, `placebo`, `carryover`)
- cumulative effects (`cumul`)
- treatment effect heterogeneity plots (`box`, `calendar`, and `hte`)
- special-purpose displays (`status`, `factors`, and `loadings`)
- standalone `esplot()`

We begin with shared parameter conventions and then work through each plot type in turn. `plot.fect` is an S3 method that accepts a fitted `fect` object and a `type` argument. All customization parameters---axis limits, colors, text sizes, reference lines---are passed as additional arguments. Some parameters apply universally; others are type-specific. A parameter applicability table appears at the end of this chapter. R script used in this chapter can be downloaded [here](https://raw.githubusercontent.com/xuyiqing/fect/dev/vignettes/rscript/11-plots.R).
...
This section shows how to *visualize* cumulative treatment effects. For the underlying estimand and the unified API that produces it, see [Chapter @sec-estimands]. The cumulative effect is only well-defined when there are no treatment reversals --- that is, all treated units remain treated for the duration of the study. The model must be estimated with `keep.sims = TRUE`.
...
## Loading-Overlap Diagnostic {#sec-loading-overlap}

The **loading.overlap** plot is the companion diagnostic for the bounded-loadings feature in v2.3.0 (see [Chapter @sec-gsynth] for the underlying simplex projection). It answers a single question that determines whether the GSC counterfactual for a treated unit is interpolated (safe) or extrapolated (risky): **do treated units lie inside the support of the controls in factor space?**
...
```{r plot-loading-overlap, fig.width = 6, fig.height = 5}
plot(out_ife, type = "loading.overlap")
```

For $r > 2$ the plot still shows only factors 1 and 2; the hull and overlap measure are computed on the full $r$-dimensional loading vectors, so the 2D scatter is a projection of the underlying multidimensional support check. Use `factors = c(i, j)` to view a different pair.

### One-dimensional case ($r = 1$)
...
- **Before deciding `loading.bound`**: the plot is informative even when `loading.bound = "none"` --- it shows whether the constraint *would* bind if activated. If most treated units are inside the hull, the constraint would have minimal effect; if many are outside, the constraint would meaningfully change the estimates.
- **As an extrapolation flag**: large `loading.proj.resid` paired with treated units far outside the hull means GSC is extrapolating beyond what the donor pool supports. Such estimates should be reported with caveats or trimmed.
- **As a model-fit diagnostic**: a treated unit far outside the hull on multiple factors may indicate that the rank `r` was set too low, or that the treated unit has a structurally different factor exposure than any control.
...
`fect()` returns a logical `T x N` matrix `$sample` marking which cells
of the panel entered the estimation (`TRUE` = used in the main fit,
placebo or carryover test, or balance check; `FALSE` = missing
observation, dropped unit, or carryover.rm exclusion). It composes
directly with **panelView** via the `panelview(fit)` entry-point:

```{r sample-fect, eval=FALSE}
library(fect); library(panelView)
data(simdata)
fit <- fect(Y ~ D, data = simdata, index = c("id", "time"),
            method = "ife", r = 1, se = FALSE, CV = FALSE)

# Pass the fit directly --- no data, formula, or index needed.
panelview(fit, type = "treat", by.timing = TRUE, sample.sort = TRUE)
```

panelview reads the panel and the mask straight from the fit object's
slots (`$Y.dat`, `$D.dat`, `$id`, `$rawtime`, `$index`, `$Y`, `$D`,
`$sample`). Cells with `sample == TRUE` use the normal palette; cells
with `sample == FALSE` render in a faded "Not used" variant. Pair with
`sample.sort = TRUE` to group the in-sample units at the top, useful
when a non-trivial subset of units get dropped by `min.T0`,
`max.missing`, or carryover-removal.

The original signature `panelview(Y ~ D, data, index, sample = fit, ...)`
still works for explicit-data workflows. See the **panelView** manual
(chapter "Treatment Status", section "Overlaying the estimation sample")
for sample-alignment rules, the `sample.sort` x `by.timing` 2x2, and
the named `color = c(control = ..., unused.control = ..., ...)` palette
syntax.
