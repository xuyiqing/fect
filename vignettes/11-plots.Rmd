...
```{r sample-fect, fig.height=12}
library(fect); library(panelView)
data(hh2019, package = "fect")

## hh2019 is otherwise complete; we inject a handful of NAs into the
## outcome to make the resulting "Not used" cells visible in the
## figure below.
set.seed(42)
hh <- hh2019
hh$nat_rate_ord[hh$bfs %in% sample(unique(hh$bfs), 30) & hh$year <= 1995] <- NA

fit <- fect(nat_rate_ord ~ indirect, data = hh,
            index = c("bfs", "year"),
            method = "ife", r = 1, se = FALSE, CV = FALSE,
            min.T0 = 2, na.rm = FALSE)

panelview(fit, type = "treat", by.timing = TRUE,
          sample.sort = TRUE, axis.lab = "off",
          display.all = TRUE)
```

In the figure, the cohort window for treated municipalities reads as a
dark-blue staircase at the top of the in-sample block, control
municipalities in their natural light blue fill the middle, and the
band of "Not used" cells at the bottom marks cells fect could not
use --- typically pre-period missing outcomes or units with too few
pre-treatment observations for the chosen rank. Pairing
`sample.sort = TRUE` with `by.timing = TRUE` groups the in-sample
units at the top and orders them by treatment timing within.
