...
The figure has three bands. The dark-blue staircase at the top is the
treated municipalities, ordered by year of treatment onset. The broad
light-blue middle band is the controls, those municipalities that
never received the treatment. The dark-grey band at the bottom is the
roughly 285 municipalities fect could not use: they were already
treated at the start of the panel, so there is no pre-treatment period
from which to estimate a counterfactual, and the estimator drops them
silently. Without the sample overlay these dropped municipalities
would be invisible.

Pairing `sample.sort = TRUE` with `by.timing = TRUE` groups the
in-sample units at the top and orders them by treatment timing within.
