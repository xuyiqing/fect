...
Default color conventions for gap plots: pre-treatment = gray (in-sample), post-treatment = black (out-of-sample). When `loo = TRUE` or `dloo = TRUE`, all points are black (all out-of-sample); the legend reads "LOO Estimates" or "Double-LOO Estimates" accordingly.
...
This section shows how to *visualize* cumulative treatment effects. The cumulative effect at event time $k$ is the sum of the per-period ATTs from event time 1 to $k$. For the underlying estimand and the unified API that produces it, see [Chapter @sec-estimands]. The cumulative effect is only well-defined when there are no treatment reversals --- that is, all treated units remain treated for the duration of the study. The model must be estimated with `keep.sims = TRUE`.
...
The **loadings** plot displays the estimated factor loadings, comparing the distribution across treated and control units. It uses a navy/crimson color scheme with smaller, semi-transparent scatter points for cleaner visualization. When multiple factors are estimated, the plot is rendered as a pairs matrix via `GGally::ggpairs`. Use `nfactors` to limit the number of factors whose loadings are shown (default: up to 4).
