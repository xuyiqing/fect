...
| **heterogenous** or **hte** | CATT by a covariate. | fe, ife, mc, gsynth, cfe |
...
|:----------|:----------|:---------:|:---------:|:---------:|:---------:|
| `W` (weight) | ✓ | ✓ | ✓ | ✓ | ✓ |
| `factors.from` (estimation sample) | – | ✓ | ✓ | ✓ | ✓ |
| `em` (EM algorithm) | – | ✓ | – | – | ✓ |
| `se` (uncertainty) | ✓ | ✓ | ✓ | ✓ | ✓ |
...
| `factors.from` | Estimation sample: "notyettreated" (default), "nevertreated". When "nevertreated", only never-treated controls estimate factors. `method = "gsynth"` auto-sets to "nevertreated". | ife, mc, cfe, gsynth |
...
| `fect_mspe()` | Model comparison via mean squared prediction error on held-out observations. | `out.fect`, `hide_n`, `n_rep`, `seed`, `pre.trend` |
| `fect_mspe_sim()` | MSPE comparison for simulated data with known counterfactuals. | `out.fect`, `hide_n`, `n_rep`, `seed` |
