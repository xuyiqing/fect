...
| `data` | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| `index` (unit & time IDs) | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| `sfe` (simple additive FEs, legacy) | – | – | – | – | – | ✓ (cfe_old) |
| `cfe` (complex FEs, legacy) | – | – | – | – | – | ✓ (cfe_old) |
...
-   `carryoverTest` is available in `fe`, `ife`, `mc`, `polynomial`, and `cfe` but not in `gsynth`. `loo` is available in `fe`, `ife`, `mc`, and `gsynth`.
