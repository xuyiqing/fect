...
List panel_factor(const arma::mat& E, int r) {
  int T = E.n_rows;
  int N = E.n_cols;
  // At most min(T, N) factors exist. A larger r (e.g. more factors than
  // control units) used to fail in head_cols(); the R callers cap r first,
  // this clamp is the second guard.
